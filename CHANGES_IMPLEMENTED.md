# v4.4.3 Netlify Function Module Fix

- Replaced `netlify/functions/api.mjs` with `netlify/functions/api.cjs`.
- The CommonJS Netlify entry point now loads the ESM application with dynamic `import()`, preventing `ERR_REQUIRE_ESM`.
- Developer credentials remain `Dvp` / `Abcd@1234`; Supabase persistence behavior is unchanged.

# v4.4.2 Developer login repair

- Developer identity is permanently normalized to `Dvp`.
- Developer password verification is fixed to the requested credential using a salted scrypt verifier; no production `DEVELOPER_BOOTSTRAP_PASSWORD` variable is required.
- Existing Supabase control-center state is normalized non-destructively; Administrator and Employee users are retained.
- Netlify sign-in forces the Developer role whenever the username is `Dvp`.
- The project Supabase URL has a safe built-in default; the Supabase secret remains server-side and must be configured for Netlify Functions.
- Added `supabase/ASSURANCE_REGENT_DEVELOPER_LOGIN_FIX.sql`.
...
- Hidden bootstrap Developer account: `Dvp` with the fixed Developer password verifier.
