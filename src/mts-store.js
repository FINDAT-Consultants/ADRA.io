...
import { uploadDataUrl } from './supabase-storage.js';
...
function dbSessionToApp(r) {
  return {
    ...
    document_name:r.document_name||'', document_type:r.document_type||'', document_size:Number(r.document_size||0), document_path:r.document_path||'', document_data:'',
...
export async function listMtsSessions({limit=1000,status,projectCode,employeeName,month,query}={}) {
  ...
  if(query){const term=query.toLowerCase();rows=rows.filter(x=>[x.employee_name,x.department,x.project_code,x.activity_description,x.delay_comments,x.clock_in_location,x.clock_out_location].join(' ').toLowerCase().includes(term));}
  return rows;
}
...
export async function clockInMtsSession({employeeId='E001',employeeName,department,projectCode,activityDescription,clockInAt,location={},document={},sessionId=''}={}) {
  ...
  const uploadedDocument=document?.data?await uploadDataUrl({data:document.data,pathPrefix:`mts/${clean(employeeId)}`,fileName:clean(document.name)||'supporting-document',contentType:clean(document.type)}):null;
  const row={
    ...
    document_name:clean(document.name), document_type:clean(document.type), document_size:Number(document.size||uploadedDocument?.size||0), document_path:uploadedDocument?.path||'', document_data:'',
  ...
  for(const row0 of normalized){
    ...
    if(bridgeCompleted && row.status==='completed' && engine.state.projects.some(p=>p.code===row.project_code) && row.duration_hours>0 && row.duration_hours<=24 && !row.recovery_entry_id){
      try{
        ...
        await createServerSupabase().from('mts_work_sessions').update({recovery_entry_id:row.recovery_entry_id,recovery_bridge_status:row.recovery_bridge_status}).eq('id',row.id);
