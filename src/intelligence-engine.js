...
async function confirmedMappingExamples() {
  const {data:rows,error}=await createServerSupabase().from('agent_learning_mappings').select('*').order('last_confirmed_at',{ascending:false});if(error)throw error;
...
export async function trainIntelligenceModels({reason='manual'}={}) {
  ...
  trainingPromise=(async()=>{
    ...
    await writeAppState('intelligence-models',state);
...
export async function initializeIntelligence() {
  state=await readAppState('intelligence-models',null);
