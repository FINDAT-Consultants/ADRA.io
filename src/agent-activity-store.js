...
import { readAppState, writeAppState } from './supabase.js';
...
async function readStore(){
  const raw=await readAppState('agent_activity',DEFAULT,{seedIfMissing:true});
  return {...structuredClone(DEFAULT),...raw,runs:raw?.runs&&typeof raw.runs==='object'?raw.runs:{},nextSeq:Number(raw?.nextSeq||1)};
}
async function writeStore(store){await writeAppState('agent_activity',store);return store;}
