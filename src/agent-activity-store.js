...
import { readAppState, writeAppState } from './supabase-state.js';
const DEFAULT={version:1,nextSeq:1,runs:{}};
const clean=v=>String(v??'').trim();
const now=()=>new Date().toISOString();

async function readStore(){const raw=await readAppState('agent-activity',DEFAULT);return {...structuredClone(DEFAULT),...raw,runs:raw?.runs&&typeof raw.runs==='object'?raw.runs:{},nextSeq:Number(raw?.nextSeq||1)};}
async function writeStore(store){await writeAppState('agent-activity',store);return store;}
