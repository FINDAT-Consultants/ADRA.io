/*
 * Assurance Regent v6.0 Recovery Assurance Engine — browser formula engine
...
(function (global) {
  ...
  function createEngine(initialState = null) {
    ...
    function recoveryRulesFor(projectCode, month, rules = []) {
      const p=projectFor(projectCode)||{}, selected=monthKey(month), donor=String(p.donor||'').trim().toLowerCase(), project=String(projectCode||'').trim().toUpperCase();
      const eligible=(Array.isArray(rules)?rules:[]).filter((r)=>{
        if(r?.active===false)return false;
        const rp=String(r?.project_code??r?.projectCode??'').trim().toUpperCase(), rd=String(r?.donor||'').trim().toLowerCase();
        if(rp && rp!==project)return false;
        if(!rp && rd && rd!==donor)return false;
        const from=monthKey(r?.effective_from||r?.effectiveFrom||''), to=monthKey(r?.effective_to||r?.effectiveTo||'');
        if(from&&selected<from)return false;if(to&&selected>to)return false;return true;
      }).sort((a,b)=>String(b?.created_at||b?.createdAt||'').localeCompare(String(a?.created_at||a?.createdAt||'')));
      const latest=new Map();
      for(const r of eligible){const scope=[String(r?.rule_key||r?.ruleKey||''),String(r?.project_code??r?.projectCode??'').trim().toUpperCase(),String(r?.donor||'').trim().toLowerCase()].join('|');if(!latest.has(scope))latest.set(scope,r);}
      return [...latest.values()];
    }

    function employeeRecoveryPassport(month, projectCode, employeeId, options = {}) {
      const selectedMonth=monthKey(month), p=projectFor(projectCode), employee=employeeFor(employeeId);
      if(!selectedMonth||!p||!employee)return null;
      const monthCal=calendarAnalysis().filter((d)=>d.month===selectedMonth), expectedHours=sum(monthCal,(d)=>d.standardHours);
      const employeeMonthEntries=state.timeEntries.filter((e)=>e.month===selectedMonth&&e.employeeId===employeeId);
      const approvedEmployeeEntries=employeeMonthEntries.filter(isFinanceApproved);
      const projectEntries=approvedEmployeeEntries.filter((e)=>e.projectCode===projectCode);
      if(!projectEntries.length && options.includeEmpty!==true)return null;
      const payroll=payrollAnalysis().filter((x)=>x.month===selectedMonth&&x.employeeId===employeeId);
      const allocableCost=sum(payroll,(x)=>x.allocableCost), hourlyCost=expectedHours?allocableCost/expectedHours:0;
      const approvedProjectHours=sum(projectEntries,(e)=>e.hours), recordedEmployeeHours=sum(approvedEmployeeEntries,(e)=>e.hours), hoursVariance=recordedEmployeeHours-expectedHours;
      const rawCost=approvedProjectHours*hourlyCost, budgetLimit=Number(p.personnelBudget||0), rules=recoveryRulesFor(projectCode,selectedMonth,options.rules||[]);
      const evidenceRows=(Array.isArray(options.evidence)?options.evidence:[]).filter((x)=>monthKey(x?.reporting_month||x?.reportingMonth)===selectedMonth&&String(x?.project_code||x?.projectCode||'').toUpperCase()===String(projectCode).toUpperCase()&&(!x?.employee_id&&!x?.employeeId||String(x?.employee_id||x?.employeeId)===String(employeeId)));
      const requiredEvidence=rules.filter((r)=>String(r.rule_key||r.ruleKey)==='EVIDENCE_REQUIRED').reduce((n,r)=>Math.max(n,Number(r.numeric_value??r.numericValue??1)||1),0);
      const maxHourly=rules.filter((r)=>String(r.rule_key||r.ruleKey)==='MAX_HOURLY_COST').map((r)=>Number(r.numeric_value??r.numericValue)).filter(Number.isFinite);
      const maxCharge=rules.filter((r)=>String(r.rule_key||r.ruleKey)==='MAX_PERSONNEL_CHARGE').map((r)=>Number(r.numeric_value??r.numericValue)).filter(Number.isFinite);
      const currencyRules=rules.filter((r)=>String(r.rule_key||r.ruleKey)==='CURRENCY').map((r)=>String(r.text_value??r.textValue??'').toUpperCase()).filter(Boolean);
      const allowAdminRules=rules.filter((r)=>String(r.rule_key||r.ruleKey)==='ALLOW_ADMIN').map((r)=>String(r.text_value??r.textValue??r.numeric_value??r.numericValue??'').trim().toLowerCase()).filter(Boolean);
      const adminAllowed=!allowAdminRules.length||allowAdminRules.every((v)=>!['0','false','no','blocked','disallow','disallowed'].includes(v));
      const adminConflict=!adminAllowed&&projectEntries.some((e)=>String(timeEntryAnalysis(e).timeType||'').toLowerCase()==='administration');
      const evidence=projectEntries.length>0 && (!requiredEvidence||evidenceRows.length>=requiredEvidence) ? 'PASS':'FAIL';
      const capacity=expectedHours>EPS&&Math.abs(hoursVariance)<EPS?'PASS':'FAIL';
      const eligibility=projectEntries.length>0&&projectEntries.every((e)=>timeEntryAnalysis(e).projectEligibility==='PASS')&&!adminConflict?'PASS':'FAIL';
      let budget=budgetLimit>0&&rawCost<=budgetLimit?'PASS':'FAIL';
      if(maxHourly.length&&hourlyCost>Math.min(...maxHourly))budget='FAIL';
      if(maxCharge.length&&rawCost>Math.min(...maxCharge))budget='FAIL';
      if(currencyRules.length&&options.currency&&!currencyRules.includes(String(options.currency).toUpperCase()))budget='FAIL';
      const approval=employeeMonthEntries.length>0&&employeeMonthEntries.every(isFinanceApproved)?'PASS':'FAIL';
      const keys={evidence,capacity,eligibility,budget,approval}, gate=Object.values(keys).every((x)=>x==='PASS')?1:0;
      const effectiveLimit=[budgetLimit,...maxCharge].filter((x)=>Number.isFinite(x)&&x>0);const limit=effectiveLimit.length?Math.min(...effectiveLimit):budgetLimit;
      const recoverableCost=gate?Math.min(rawCost,limit||rawCost):0, amountAtRisk=Math.max(0,rawCost-recoverableCost);
      const keyReasons={
        evidence:evidence==='PASS'?`${projectEntries.length} approved work entr${projectEntries.length===1?'y':'ies'}${requiredEvidence?` and ${evidenceRows.length}/${requiredEvidence} linked evidence item(s)`:''}.`:requiredEvidence&&evidenceRows.length<requiredEvidence?`Donor rule requires ${requiredEvidence} supporting evidence item(s); ${evidenceRows.length} linked.`:'No finance-approved work evidence is available for this employee/project.',
        capacity:capacity==='PASS'?`Recorded employee hours reconcile to ${round(expectedHours)} expected hours.`:`Employee has ${round(recordedEmployeeHours)} approved hours against ${round(expectedHours)} expected hours (${round(hoursVariance)} variance).`,
        eligibility:eligibility==='PASS'?'All approved project entries pass project/date/employee eligibility and applicable donor administration rules.':adminConflict?'A donor rule disallows administration time on this funding objective.':'At least one approved entry is outside project/date/employee eligibility.',
        budget:budget==='PASS'?`Proposed personnel charge is within the applicable budget/rule ceiling.`:`Proposed charge ${round(rawCost,2)} exceeds an applicable project/donor ceiling or currency rule.`,
        approval:approval==='PASS'?'All employee time evidence in the month is finance approved.':'At least one monthly time entry is not finance approved.'
      };
      const remediation=[];
      if(evidence!=='PASS')remediation.push({owner:requiredEvidence?'Employee / Finance':'Employee',action:requiredEvidence?`Link at least ${Math.max(0,requiredEvidence-evidenceRows.length)} additional supporting evidence item(s) to this recovery record.`:'Record and submit the missing project work evidence.'});
      if(capacity!=='PASS')remediation.push({owner:'Employee',action:hoursVariance<0?`Complete ${round(Math.abs(hoursVariance))} missing expected hour(s), then obtain the required reapprovals.`:`Resolve ${round(Math.abs(hoursVariance))} excess hour(s) against expected capacity, then reapprove the period.`});
      if(eligibility!=='PASS')remediation.push({owner:'Project / Programs Manager',action:adminConflict?'Move administration work to an authorized administrative funding objective or obtain a formal donor-rule change; do not override the restriction.':'Correct the project/date/employee assignment or move the work to an eligible funding objective; do not override the donor restriction.'});
      if(budget!=='PASS')remediation.push({owner:'Finance / Programs',action:'Resolve the donor/project budget or ceiling exception through an authorized budget/rule change or use another eligible funding source.'});
      if(approval!=='PASS')remediation.push({owner:'Supervisor / Finance',action:'Complete the outstanding human approval chain for the monthly time evidence.'});
      const payrollConfig=payroll.length&&payroll.every((x)=>String(x.configurationStatus||'').toUpperCase()!=='REVIEW')?'COMPLETE':'REVIEW';
      const monthControl=monthlyEngine().find((x)=>x.month===selectedMonth),sourceHours=Number(monthControl?.sourceHours||0),sourceDelta=Number(monthControl?.sourceReconciliation||0);
      const sourceConfigured=Boolean(state.sourceChecks.find((x)=>x.month===selectedMonth));
      const controls=[
        {control:'Employee monthly hours variance',value:round(hoursVariance),result:capacity==='PASS'?'PASS':'FAIL',action:capacity==='PASS'?'Expected capacity reconciles.':'Resolve missing/excess time and reapprove the period.'},
        {control:'Project / donor eligibility',value:eligibility,result:eligibility,action:eligibility==='PASS'?'All entries are eligible.':'Correct the project/date/employee assignment; do not override the donor restriction.'},
        {control:'Supporting evidence',value:`${evidenceRows.length}/${requiredEvidence||0}`,result:evidence,action:evidence==='PASS'?'Evidence requirement satisfied.':'Link the required supporting evidence.'},
        {control:'Payroll configuration',value:payrollConfig,result:payrollConfig==='COMPLETE'?'PASS':'REVIEW',action:payrollConfig==='COMPLETE'?'Employee payroll is configured.':'Complete benefits/statutory/exclusion configuration.'},
        {control:'Source reconciliation',value:sourceDelta,result:sourceConfigured?(Math.abs(sourceDelta)<EPS?'PASS':'FAIL'):'REVIEW',action:sourceConfigured?(Math.abs(sourceDelta)<EPS?'Source hours reconcile at period level.':'Resolve the period-level source-hours difference.'):'No source check exists for this period.'}
      ];
      return {reportingMonth:selectedMonth,projectCode,projectName:p.name||'',donor:p.donor||'',employeeId,employee:employee.name||employeeId,position:employee.position||'',approvedProjectHours:round(approvedProjectHours),expectedHours:round(expectedHours),recordedEmployeeHours:round(recordedEmployeeHours),hoursVariance:round(hoursVariance),allocableCost:round(allocableCost,2),hourlyCost:round(hourlyCost,8),rawCost:round(rawCost,2),budgetLimit:round(budgetLimit,2),keys,keyReasons,recoveryGate:gate,recoverableCost:round(recoverableCost,2),amountAtRisk:round(amountAtRisk,2),finalStatus:gate?'RECOVERABLE':'BLOCKED',evidenceCount:evidenceRows.length,requiredEvidence,appliedRules:rules,remediation,controls,payrollConfig,sourceHours,sourceDelta};
    }

    function recoveryPassports(month, options={}) {
      const selected=monthKey(month), seen=new Set(), rows=[];
      for(const e of state.timeEntries.filter((x)=>x.month===selected&&x.projectCode&&x.employeeId)){
        const key=`${e.employeeId}__${e.projectCode}`;if(seen.has(key))continue;seen.add(key);const row=employeeRecoveryPassport(selected,e.projectCode,e.employeeId,options);if(row)rows.push(row);
      }
      const byProject=new Map();for(const r of rows){if(!byProject.has(r.projectCode))byProject.set(r.projectCode,[]);byProject.get(r.projectCode).push(r);}for(const group of byProject.values()){const total=sum(group,(r)=>r.rawCost),limit=Number(group[0]?.budgetLimit||0);if(limit>0&&total>limit){for(const r of group){r.keys.budget='FAIL';r.keyReasons.budget=`Combined proposed personnel charges for ${r.projectCode} are ${round(total,2)} against a project personnel budget of ${round(limit,2)}.`;r.remediation=[...(r.remediation||[]).filter((x)=>x.owner!=='Finance / Programs'),{owner:'Finance / Programs',action:'Resolve the aggregate project personnel-budget exception before recovery.'}];r.recoveryGate=0;r.finalStatus='BLOCKED';r.recoverableCost=0;r.amountAtRisk=r.rawCost;}}}
      return rows.sort((a,b)=>b.amountAtRisk-a.amountAtRisk||String(a.employee).localeCompare(String(b.employee)));
    }

    function recoveryPassport(month, projectCode, options={}) {
      const selected=monthKey(month), candidates=[...new Set(state.timeEntries.filter((e)=>e.month===selected&&e.projectCode===projectCode).map((e)=>e.employeeId).filter(Boolean))];
      const employeeId=String(options.employeeId||candidates[0]||String(projectFor(projectCode)?.eligibleEmployeeId||'').split(',')[0]||state.employees[0]?.employeeId||'').trim();
      return employeeId?employeeRecoveryPassport(selected,projectCode,employeeId,options):null;
    }
    ...
    function upsertEmployee(value) {
      ...
      const row = {
        companyId:String(value.companyId??existing.companyId??'COMPANY-DEFAULT').trim()||'COMPANY-DEFAULT',
        employeeId, name:String(value.name??existing.name??'').trim(), position:pick('position'), supervisor:pick('supervisor'),
        department:pick('department'), team:pick('team'), email:pick('email'), phone:pick('phone'), skype:pick('skype'), profilePhoto:pick('profilePhoto'), profilePhotoFileId:pick('profilePhotoFileId'),
    ...
    function upsertProject(value) {
      ...
      const row = { companyId:String(value.companyId??existing.companyId??'COMPANY-DEFAULT').trim()||'COMPANY-DEFAULT', code, name:String(value.name??existing.name??value.code??'').trim(), donor:String(value.donor??existing.donor??'').trim(), startDate:excelDate(value.startDate??existing.startDate), endDate:excelDate(value.endDate??existing.endDate), status:value.status ?? existing.status ?? 'Active', adminAllowed:value.adminAllowed ?? existing.adminAllowed ?? 'No', personnelBudget:Number(value.personnelBudget??existing.personnelBudget??0), eligibleEmployeeId:String(value.eligibleEmployeeId??existing.eligibleEmployeeId??'').trim(), createdAt:existing.createdAt||value.createdAt||new Date().toISOString(), updatedAt:new Date().toISOString() };
    ...
    function upsertPayroll(value) {
      ...
      const row = { companyId:String(value.companyId??existing.companyId??'COMPANY-DEFAULT').trim()||'COMPANY-DEFAULT', month, employeeId, basicSalary:Number(value.basicSalary??existing.basicSalary??0), benefits:Number(value.benefits??existing.benefits??0), statutoryCost:Number(value.statutoryCost??existing.statutoryCost??0), exclusions:Number(value.exclusions??existing.exclusions??0), source:String(value.source??existing.source??'User input').trim(), configurationStatus:value.configurationStatus ?? existing.configurationStatus ?? 'COMPLETE', notes:String(value.notes??existing.notes??'').trim(), createdAt:existing.createdAt||value.createdAt||new Date().toISOString(), updatedAt:new Date().toISOString() };
    ...
    function upsertCalendar(value) {
      ...
      const row = { companyId:String(value.companyId??existing.companyId??'COMPANY-DEFAULT').trim()||'COMPANY-DEFAULT', date, month:monthKey(date), day:value.day ?? existing.day ?? (date ? new Date(`${date}T00:00:00Z`).toLocaleDateString('en',{weekday:'short',timeZone:'UTC'}) : ''), dayType:value.dayType ?? existing.dayType ?? 'Working Day', standardHours:Number(value.standardHours??existing.standardHours??0), holidaySource:String(value.holidaySource??existing.holidaySource??'').trim(), createdAt:existing.createdAt||value.createdAt||new Date().toISOString(), updatedAt:new Date().toISOString() };
    ...
    function upsertVacancy(value) {
      ...
      const row={...existing,id,companyId:String(value.companyId??existing.companyId??'COMPANY-DEFAULT').trim()||'COMPANY-DEFAULT',title:String(value.title??existing.title??'').trim(),department:String(value.department??existing.department??'').trim(),location:String(value.location??existing.location??'').trim(),employmentType:String(value.employmentType??existing.employmentType??'Full Time').trim(),status:String(value.status??existing.status??'Open').trim(),openDate:excelDate(value.openDate??existing.openDate??new Date().toISOString().slice(0,10)),closeDate:excelDate(value.closeDate??existing.closeDate),publicVisible:Boolean(value.publicVisible??existing.publicVisible??false),publicDescription:String(value.publicDescription??existing.publicDescription??'').trim().slice(0,10000),publicRequirements:String(value.publicRequirements??existing.publicRequirements??'').trim().slice(0,10000),publicResponsibilities:String(value.publicResponsibilities??existing.publicResponsibilities??'').trim().slice(0,10000),publicSkills:String(value.publicSkills??existing.publicSkills??'').trim().slice(0,6000),publicEducation:String(value.publicEducation??existing.publicEducation??'').trim().slice(0,3000),publicBenefits:String(value.publicBenefits??existing.publicBenefits??'').trim().slice(0,5000),publicAboutCompany:String(value.publicAboutCompany??existing.publicAboutCompany??'').trim().slice(0,5000),experienceYears:Math.max(0,Number(value.experienceYears??existing.experienceYears??0)||0),salaryRange:String(value.salaryRange??existing.salaryRange??'').trim().slice(0,300),reportingLine:String(value.reportingLine??existing.reportingLine??'').trim().slice(0,300),positionsCount:Math.max(1,Math.min(999,Number(value.positionsCount??existing.positionsCount??1)||1)),jobReference:String(value.jobReference??existing.jobReference??'').trim().slice(0,160),companyLogo:String(value.companyLogo??existing.companyLogo??'').trim().slice(0,350000),companyLogoFileId:String(value.companyLogoFileId??existing.companyLogoFileId??'').trim(),archived:Boolean(value.archived??existing.archived??false),archivedAt:String(value.archivedAt??existing.archivedAt??''),createdAt:existing.createdAt||value.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};
    ...
    function upsertCandidate(value) {
      ...
      const row={...existing,id,companyId:String(value.companyId??existing.companyId??'COMPANY-DEFAULT').trim()||'COMPANY-DEFAULT',name:String(value.name??existing.name??'').trim(),email:String(value.email??existing.email??'').trim(),phone:String(value.phone??existing.phone??'').trim(),vacancyId:String(value.vacancyId??existing.vacancyId??'').trim(),jobTitle:String(value.jobTitle??existing.jobTitle??'').trim(),department:String(value.department??existing.department??'').trim(),location:String(value.location??existing.location??'').trim(),employmentType:String(value.employmentType??existing.employmentType??'Full Time').trim(),stage:String(value.stage??existing.stage??'Applied').trim(),status:String(value.status??existing.status??'Active').trim(),profilePhoto:String(value.profilePhoto??existing.profilePhoto??'').trim(),profilePhotoFileId:String(value.profilePhotoFileId??existing.profilePhotoFileId??'').trim(),appliedDate:excelDate(value.appliedDate??existing.appliedDate??new Date().toISOString().slice(0,10)),notes:String(value.notes??existing.notes??'').trim(),createdAt:existing.createdAt||value.createdAt||new Date().toISOString(),updatedAt:new Date().toISOString()};
    ...
    function upsertSourceCheck(value) {
      const existing=state.sourceChecks.find(x=>x.month===monthKey(value.month))||{};const row = { month:monthKey(value.month), check:value.check || 'Work Activity Hub completed hours', sourceTarget:Number(value.sourceTarget||0), severity:value.severity || 'Critical', whereToFix:value.whereToFix || 'Work Activity Hub / Recovery Time Entry', createdAt:existing.createdAt||value.createdAt||new Date().toISOString(), updatedAt:new Date().toISOString() };
    ...
    function addTimeEntry(entry) {
      ...
      const row = {
        companyId:String(entry.companyId??employee.companyId??project.companyId??'COMPANY-DEFAULT').trim()||'COMPANY-DEFAULT',
        ...
        aiSuggestedProject: entry.aiSuggestedProject || project.code, aiConfidence: Number(entry.aiConfidence ?? 1), employeeDecision: entry.employeeDecision || 'Accepted', createdAt:entry.createdAt||new Date().toISOString(), updatedAt:new Date().toISOString()
      };
    ...
    return {
      ...
      analyzedTimeEntries: () => state.timeEntries.map(timeEntryAnalysis), checks: () => checks(monthlyEngine()), voucher, recoveryPassport, employeeRecoveryPassport, recoveryPassports, recoveryRulesFor, projectSummary,
