// Assurance Regent v5.9.0 Jivan — named AI operator, delegated background tasks, leave/work-location operations, voice and research
...
function toolsFor(profile: any, pageContext: any, context: any, backgroundMode = false) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const history=(Array.isArray(thread)?thread:[]).slice(-16).map((m:any)=>`${m.role==='user'?'User':'Jivan'}: ${String(m.content||'').slice(0,2400)}`).join('\n');
    ...
    const instructions=`YOUR NAME IS JIVAN. You are Jivan v5.9 inside Assurance Regent, an interactive software operator and named agentic AI, not a human. Treat “Jivan”, “Hey Jivan”, “Hi Jivan”, and “Okay Jivan” as direct address. If the user says only “Jivan”, acknowledge briefly and ask what they need. ${tierInstruction}
...
${message}`;
    ...
    await audit(sessionToken,'COMMAND',backgroundMode?'background_received':'received',String(pageContext?.view||''),'OK',message.slice(0,1000),{role:profile.role});
    const outputTokenLimit=backgroundMode?(profile.role==='Developer'?3600:profile.role==='Administrator'?3000:2200):(profile.role==='Developer'?3200:profile.role==='Administrator'?2600:1800);
    const openaiResponse=await fetch('https://api.openai.com/v1/responses',{method:'POST',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},body:JSON.stringify({model,instructions,input,tools:toolsFor(profile,pageContext,context,backgroundMode),tool_choice:'auto',store:false,max_output_tokens:outputTokenLimit})});
    ...
    if(!continuation&&!backgroundMode)await rpc('assurance_regent_browser_agent_append',{p_token:sessionToken,p_role:'user',p_content:message,p_source:'conversation',p_metadata:{model,request_id:openai?.id||'',page:pageContext?.view||'',role:profile.role}});
    if(!continueTask)await rpc('assurance_regent_browser_agent_append',{p_token:sessionToken,p_role:'assistant',p_content:answer,p_source:backgroundMode?'task_completion':'conversation',p_metadata:{model,response_id:openai?.id||'',ui_actions:uiActions,role:profile.role,continuation_step:continuationStep,background_task:Boolean(backgroundMode),task_id:String(body?.task_id||'')}});
    ...
    return json({ok:true,output_text:answer,model,llm_configured:true,requires_approval:false,executed_actions:[],ui_actions:uiActions,continue_task:continueTask,background_mode:backgroundMode,advisory_only:false,role_profile:{role:profile.role,level:profile.level,label:profile.label},capabilities:profile.capabilities});
  } catch (error) {
    console.error('Jivan v5.9 error:',error);const message=error instanceof Error?error.message:String(error||'Jivan failed.');if(sessionToken)await audit(sessionToken,'ERROR','request','','ERROR',message,{});const status=/session has expired|signed-in/i.test(message)?401:500;return json({error:message},status);
  }
});
