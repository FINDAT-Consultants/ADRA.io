// Assurance Regent v6.3.80 — Developer-managed Google Meet Media API connector
...
function isDeveloper(ctx:any){return String(ctx?.actor?.id||'')===PLATFORM_ACTOR_ID||String(ctx?.authority||'').toUpperCase()==='DEVELOPER'||String(ctx?.actor?.role||'').toLowerCase()==='developer';}
function hasScope(c:any,scope:string){return Boolean(c&&String(c.granted_scope||'').split(/\s+/).includes(scope));}
...
function popupResult(returnTo:string,status:string,message=''){
...
async function verifyContext(token:string){if(!token)throw new Error('Sign in to Assurance Regent before using Live Meet Assistant.');const ctx=await browserRpc('assurance_regent_browser_agent_context',{p_token:token});if(!ctx?.actor?.id)throw new Error('The signed-in Assurance Regent user could not be verified.');return ctx;}
...
async function upsertConnection(row:any){const rows=await serviceFetch('/rest/v1/assurance_regent_meet_media_connections?on_conflict=actor_id',{method:'POST',headers:{'Content-Type':'application/json',Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({...row,actor_id:PLATFORM_ACTOR_ID})});return rows?.[0]||row;}
async function insertState(row:any){await serviceFetch('/rest/v1/assurance_regent_meet_media_oauth_states',{method:'POST',headers:{'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify(row)});}
async function getState(hash:string){const rows=await serviceFetch(`/rest/v1/assurance_regent_meet_media_oauth_states?state_hash=eq.${encodeURIComponent(hash)}&select=*&limit=1`,{headers:{Accept:'application/json'}});return rows?.[0]||null;}
async function markStateUsed(hash:string){await serviceFetch(`/rest/v1/assurance_regent_meet_media_oauth_states?state_hash=eq.${encodeURIComponent(hash)}`,{method:'PATCH',headers:{'Content-Type':'application/json',Prefer:'return=minimal'},body:JSON.stringify({used_at:new Date().toISOString()})});}
...
async function createAuthorization(ctx:any,returnTo:any){
...
async function googleUser(accessToken:string){const r=await fetch('https://openidconnect.googleapis.com/v1/userinfo',{headers:{Authorization:`Bearer ${accessToken}`}});const t=await r.text();let b:any={};try{b=t?JSON.parse(t):{};}catch{}if(!r.ok)throw new Error(b?.error_description||b?.error||`Google account lookup failed (${r.status}).`);return b;}
async function getSpace(meetingCode:string,accessToken:string){const r=await fetch(`https://meet.googleapis.com/v2/spaces/${encodeURIComponent(meetingCode)}`,{headers:{Authorization:`Bearer ${accessToken}`}});const t=await r.text();let b:any={};try{b=t?JSON.parse(t):{};}catch{b={error:{message:t}};}if(!r.ok)throw new Error(b?.error?.message||b?.message||`Google Meet space lookup failed (${r.status}).`);return b;}
...
async function recruitmentInterview(token:string,id:string){const bundle=await browserRpc('assurance_regent_browser_recruitment_bundle',{p_token:token}),row=(bundle?.interviews||[]).find((x:any)=>String(x.id||'')===String(id||''));return {bundle,row};}
...
Deno.serve(async(req:any)=>{
  ...
  try{
    ...
    if(action==='session'){
      ...
      let access:any;try{access=await refreshAccess(String(c.refresh_token));}catch(err:any){return json({error:'Live Meet Assistant authorization expired. Contact the Developer to reconnect it.',code:'MEET_MEDIA_RECONNECT_REQUIRED',detail:String(err?.message||err).slice(0,240)},409);}const accessToken=clean(access?.access_token,5000);if(!accessToken)return json({error:'Google did not return a Live Meet access token.'},502);
      const space=await getSpace(code,accessToken),spaceName=clean(space?.name,300),spaceId=spaceName.startsWith('spaces/')?spaceName.slice(7):spaceName;if(!spaceId)return json({error:'Google Meet did not return a meeting space ID.'},502);if(!space?.activeConference)return json({error:'The Google Meet room is not currently active. Join/start the interview room first, then bring Jivan live.',code:'NO_ACTIVE_CONFERENCE'},409);
      await touchConnection();return json({ok:true,ready:true,meeting_space_id:spaceId,space_name:spaceName,meeting_code:clean(space?.meetingCode||code,160),active_conference:space?.activeConference?.conferenceRecord||space?.activeConference?.name||space?.activeConference||null,access_token:accessToken,expires_in:Number(access?.expires_in||3600),token_type:clean(access?.token_type||'Bearer',40),audio_only:true,shared:true,authenticated_principal_must_be_present:true});
    }
    return json({error:'Unknown Live Meet Media action.'},400);
  }catch(err:any){const m=String(err?.message||err||'Live Meet Media connector failed.');const status=/sign in|session|verified/i.test(m)?401:/authority|another company|not permitted/i.test(m)?403:/not configured/i.test(m)?503:400;return json({error:m},status);}
});
