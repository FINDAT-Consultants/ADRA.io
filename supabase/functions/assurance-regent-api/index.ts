 ...
 *
 * Required Edge Function secret for AI features:
 *   OPENAI_API_KEY
 *
 * The Developer account is fixed as DVP and its requested password is represented only
 * by a salted scrypt hash in Supabase / this function; no bootstrap password secret is required.
 * Optional:
...
function serviceKey(){
  // Hosted Supabase Edge Functions automatically expose the project's server keys.
  // Prefer the new sb_secret_* key map, then fall back to legacy service_role only
  // for projects that have not fully migrated yet.
  try{
  ...
  throw new Error('Supabase privileged Edge credential is unavailable. Confirm SUPABASE_SECRET_KEYS exists in Edge Function Secrets.');
}
function publishableKeys(){
  const keys:string[]=[];
  try{
    const parsed=JSON.parse(Deno.env.get('SUPABASE_PUBLISHABLE_KEYS')||'{}');
    for(const v of Object.values(parsed)) if(typeof v==='string'&&String(v).trim()) keys.push(String(v).trim());
  }catch{}
  const legacy=Deno.env.get('SUPABASE_ANON_KEY');
  if(legacy?.trim()) keys.push(legacy.trim());
  return [...new Set(keys)];
}
function validClientApiKey(req:Request){
  const supplied=clean(req.headers.get('apikey'));
  if(!supplied) return false;
  const allowed=publishableKeys();
  // Fail closed when Supabase did not inject a browser key into the function.
  return allowed.length>0 && allowed.includes(supplied);
}
...
const DVP_PASSWORD_HASH='78940965ef0d57a04b48066a5dbb118d:ce5a3b62726cb2e91ec1456d0b9ef34ff546c9c4f747c856aca4fa02ea90663a35f2d74acf3496a8401f8c42bc38e28ab7799bb8943bc764e5018e990d773a1a';
...
function controlDefault(){
  return {
    ...
    companies:[
      {id:'COMPANY-DEFAULT',name:'System Workspace',code:'DEFAULT',active:true,hidden:true,system:true,createdAt:now(),createdBy:'DVP'},
      {id:'COMPANY-ASSURANCE-REGENT',name:'Assurance Regent',code:'ASSURANCE',active:true,hidden:false,system:false,createdAt:now(),createdBy:'DVP'}
    ],
    users:[{id:'DVP',username:'DVP',name:'Developer',email:'',position:'System Developer',companyId:'',role:ROLE_DEVELOPER,profilePhoto:'',passwordHash:DVP_PASSWORD_HASH,hiddenFromDirectory:true,active:true,createdAt:now(),createdBy:'SYSTEM'}],
    documents:[],reviews:[],reviewResolutions:{}
  };
}
function userFromRow(r:any){return {id:r.id,username:r.username,name:r.name,email:r.email||'',position:r.position||'',department:r.department||'',supervisor:r.supervisor||'',supervisoryRole:r.supervisory_role||'',companyId:r.company_id||'',role:r.role,profilePhoto:r.profile_photo||'',passwordHash:r.password_hash||'',hiddenFromDirectory:Boolean(r.hidden_from_directory),active:r.active!==false,createdAt:r.created_at||now(),createdBy:r.created_by||''};}
function userToRow(u:any){return {id:clean(u.id),username:clean(u.username||u.id),email:lower(u.email),name:clean(u.name||u.username||u.id),position:clean(u.position),department:clean(u.department),supervisor:clean(u.supervisor),supervisory_role:clean(u.supervisoryRole),company_id:clean(u.companyId),role:clean(u.role)||ROLE_EMPLOYEE,profile_photo:clean(u.profilePhoto),password_hash:clean(u.passwordHash),hidden_from_directory:Boolean(u.hiddenFromDirectory),active:u.active!==false,created_by:clean(u.createdBy),created_at:u.createdAt||now(),updated_at:now()};}
function companyFromRow(r:any){return {id:r.id,name:r.name,code:r.code,active:r.active!==false,hidden:Boolean(r.hidden),system:Boolean(r.system),createdAt:r.created_at||now(),createdBy:r.created_by||''};}
function companyToRow(c:any){return {id:clean(c.id),name:clean(c.name),code:clean(c.code).toUpperCase(),active:c.active!==false,hidden:Boolean(c.hidden),system:Boolean(c.system),created_by:clean(c.createdBy),created_at:c.createdAt||now(),updated_at:now()};}
async function assertNoDbError(label:string,result:any){if(result?.error)fail(`${label}: ${result.error.message}. Run the latest Assurance Regent SQL setup in Supabase SQL Editor.`,503);return result;}
async function syncJsonRows(table:string,rows:any[]){
  const normalized=(rows||[]).filter(x=>clean(x?.id)).map(x=>({id:clean(x.id),row_data:x,updated_at:now()}));
  if(normalized.length){const r=await db.from(table).upsert(normalized,{onConflict:'id'});await assertNoDbError(`Could not persist ${table}`,r);}
  const existing=await db.from(table).select('id');await assertNoDbError(`Could not read ${table}`,existing);
  const wanted=new Set(normalized.map(x=>x.id)),stale=(existing.data||[]).map((x:any)=>x.id).filter((id:string)=>!wanted.has(id));
  if(stale.length){const r=await db.from(table).delete().in('id',stale);await assertNoDbError(`Could not reconcile ${table}`,r);}
}
async function writeControl(c:any){
  const users=(c.users||[]).map(userToRow),companies=(c.companies||[]).map(companyToRow);
  if(users.length){const r=await db.from('app_users').upsert(users,{onConflict:'id'});await assertNoDbError('Could not persist users',r);}
  if(companies.length){const r=await db.from('app_companies').upsert(companies,{onConflict:'id'});await assertNoDbError('Could not persist companies',r);}
  const existingUsers=await db.from('app_users').select('id');await assertNoDbError('Could not reconcile users',existingUsers);
  const wantedUsers=new Set(users.map((x:any)=>x.id));const staleUsers=(existingUsers.data||[]).map((x:any)=>x.id).filter((id:string)=>!wantedUsers.has(id));if(staleUsers.length){const r=await db.from('app_users').delete().in('id',staleUsers);await assertNoDbError('Could not delete removed users',r);}
  const existingCompanies=await db.from('app_companies').select('id');await assertNoDbError('Could not reconcile companies',existingCompanies);
  const wantedCompanies=new Set(companies.map((x:any)=>x.id));const staleCompanies=(existingCompanies.data||[]).map((x:any)=>x.id).filter((id:string)=>!wantedCompanies.has(id));if(staleCompanies.length){const r=await db.from('app_companies').delete().in('id',staleCompanies);await assertNoDbError('Could not delete removed companies',r);}
  const settings=await db.from('app_settings').upsert({id:'global',settings:c.settings||{},updated_at:now()},{onConflict:'id'});await assertNoDbError('Could not persist settings',settings);
  await syncJsonRows('app_documents',c.documents||[]);await syncJsonRows('app_reviews',c.reviews||[]);
  await writeState('control_center',c); // Supabase JSONB compatibility backup; never browser-local.
  return c;
}
async function readControl(){
  const fallback=controlDefault();
  const [ur,cr,sr,dr,rr]=await Promise.all([
    db.from('app_users').select('*').order('created_at',{ascending:true}),
    db.from('app_companies').select('*').order('created_at',{ascending:true}),
    db.from('app_settings').select('settings').eq('id','global').maybeSingle(),
    db.from('app_documents').select('row_data').order('updated_at',{ascending:false}),
    db.from('app_reviews').select('row_data').order('updated_at',{ascending:false})
  ]);
  for(const [label,res] of [['app_users',ur],['app_companies',cr],['app_settings',sr],['app_documents',dr],['app_reviews',rr]] as any[])await assertNoDbError(`Could not read ${label}`,res);
  let c:any={...fallback,users:(ur.data||[]).map(userFromRow),companies:(cr.data||[]).map(companyFromRow),settings:{...fallback.settings,...(sr.data?.settings||{})},documents:(dr.data||[]).map((x:any)=>x.row_data).filter(Boolean),reviews:(rr.data||[]).map((x:any)=>x.row_data).filter(Boolean),reviewResolutions:{}};
  // One-time migration from the previous Supabase JSONB control-center state.
  if(!c.users.length){const legacy=await db.from('app_state_documents').select('state_value').eq('state_key','control_center').maybeSingle();if(!legacy.error&&legacy.data?.state_value)c={...c,...legacy.data.state_value,settings:{...fallback.settings,...(legacy.data.state_value.settings||{})}};}
  let changed=false;
  let dev=(c.users||[]).find((u:any)=>lower(u.username||u.id)==='dvp'||u.role===ROLE_DEVELOPER);
  if(!dev){dev=clone(fallback.users[0]);c.users.unshift(dev);changed=true;}else{
    const oldId=dev.id;Object.assign(dev,{id:'DVP',username:'DVP',name:dev.name||'Developer',position:dev.position||'System Developer',role:ROLE_DEVELOPER,passwordHash:DVP_PASSWORD_HASH,hiddenFromDirectory:true,active:true});
    if(oldId!=='DVP'||dev.passwordHash!==DVP_PASSWORD_HASH)changed=true;
  }
  for(const seed of fallback.companies){if(!c.companies.some((x:any)=>x.id===seed.id||lower(x.code)===lower(seed.code))){c.companies.push(seed);changed=true;}}
  c.settings={...fallback.settings,...(c.settings||{})};c.documents=Array.isArray(c.documents)?c.documents:[];c.reviews=Array.isArray(c.reviews)?c.reviews:[];c.reviewResolutions=c.reviewResolutions||{};
  // Always reconcile normalized Supabase tables so user creation is directly visible in Table Editor.
...
async function saveLive(s:any){
  await writeState('live_system_data',s);
  const rows:any[]=[];
  for(const [record_type,list] of Object.entries(s||{})) if(Array.isArray(list)) for(const item of list as any[]){
    const record_id=clean(item.id||item.entryId||item.employeeId||item.code||item.date||item.month||`${record_type}-${rows.length}`);
    rows.push({record_type,record_id,company_id:clean(item.companyId||item.company_id)||'COMPANY-DEFAULT',data:item,updated_at:now()});
  }
  if(rows.length){const r=await db.from('app_live_records').upsert(rows,{onConflict:'record_type,record_id,company_id'});await assertNoDbError('Could not persist live records',r);}
  return s;
}
...
async function handle(req:Request,e:Envelope){
  ...
  if(path==='/api/health') {const c=await readControl(),live=await liveState();await saveLive(live);const [uCount,cCount,stateCount,liveCount]=await Promise.all([db.from('app_users').select('*',{count:'exact',head:true}),db.from('app_companies').select('*',{count:'exact',head:true}),db.from('app_state_documents').select('*',{count:'exact',head:true}),db.from('app_live_records').select('*',{count:'exact',head:true})]);return {status:200,body:{ok:true,runtime:'supabase-edge-only',database:'supabase-postgres',storage:'supabase-storage',persistence:{users:uCount.count||c.users.length,companies:cCount.count||c.companies.length,state_documents:stateCount.count||0,live_records:liveCount.count||0,developer_ready:c.users.some((u:any)=>u.id==='DVP')},openai:{configured:Boolean(Deno.env.get('OPENAI_API_KEY')),runtime:'edge-function',model:Deno.env.get('OPENAI_MODEL')||'gpt-5.4'}}};}

  if(path==='/api/control-center/login'&&method==='POST'){
    const c=await readControl(),identifier=clean(body.identifier||body.username||body.email||body.userId),password=clean(body.password);
    if(!identifier||!password)fail('Username or email address and password are required.');
    const needle=lower(identifier);
    const user=c.users.find((u:any)=>u.active!==false&&[u.username,u.id,u.email].some(v=>lower(v)===needle));
    if(!user||!verifyPassword(password,user.passwordHash))fail('Invalid username/email or password.',401);
  ...
  if(path==='/api/control-center/register'&&method==='POST'){
    const c=await readControl(),id=clean(body.userId||body.username),role=clean(body.role||body.userType)||ROLE_EMPLOYEE,password=clean(body.password),name=clean(body.name),position=clean(body.position),companyCode=clean(body.companyCode),email=lower(body.email);
    if(!id||!email||!password||!name||!position||!companyCode)fail('Username, email address, company code, name, position and password are required.');
    ...
    if(c.users.some((u:any)=>[u.id,u.username].some(v=>lower(v)===lower(id))))fail('That username is already registered.');
    if(c.users.some((u:any)=>email&&lower(u.email)===email))fail('That email address is already registered.');
    const company=c.companies.find((x:any)=>lower(x.code)===lower(companyCode)&&x.active!==false&&!x.hidden);if(!company)fail('A valid company code is required. For the Assurance Regent workspace use ASSURANCE.');
    ...
    const user={id,username:id,name,email,position,department:'',supervisor:'',supervisoryRole:'',companyId:company.id,role,profilePhoto:'',passwordHash:hashPassword(password),hiddenFromDirectory:false,active:true,createdAt:now()};
    c.users.push(user);await writeControl(c);
  ...
  if(path==='/api/control-center/settings'&&method==='PATCH'){
    requireRole(actor,[ROLE_DEVELOPER,ROLE_ADMIN]);const c=await readControl();c.settings={...c.settings,...body};await writeControl(c);return {status:200,body:c.settings};
  }
  if(path==='/api/control-center/profile'&&method==='PATCH'){
    ...
    await writeControl(c);return {status:200,body:publicUser(u)};
  }
  if(path==='/api/control-center/companies'&&method==='POST'){
    requireRole(actor,[ROLE_DEVELOPER]);const c=await readControl(),name=clean(body.name||body.companyName),code=clean(body.code||body.companyCode).toUpperCase();if(!name||!code)fail('Company name and code are required.');if(c.companies.some((x:any)=>lower(x.code)===lower(code)))fail('Company code already exists.');const row={id:uid('COMPANY'),name,code,active:true,hidden:false,system:false,createdAt:now(),createdBy:actor.id};c.companies.push(row);await writeControl(c);return {status:201,body:row};
  }
  if(path==='/api/control-center/companies/executive'&&method==='POST'){
    ...
    const user={id:userId,username:userId,name:clean(body.executiveName||body.fullName||body.employeeName)||execName||userId,email:clean(body.email),position,department:clean(body.department),companyId:company.id,role:ROLE_ADMIN,profilePhoto:'',passwordHash:hashPassword(password),hiddenFromDirectory:false,active:true,createdAt:now(),createdBy:actor.id};c.users.push(user);await writeControl(c);return {status:201,body:{company,user:publicUser(user)}};
  }
  if(path==='/api/control-center/users'&&method==='POST'){
    requireRole(actor,[ROLE_DEVELOPER]);const c=await readControl(),id=clean(body.userId||body.username),password=clean(body.password),companyId=clean(body.companyId),role=clean(body.role)||ROLE_EMPLOYEE,email=lower(body.email);if(!id||!email||password.length<8||!companyId)fail('Company, username, email address and an 8-character password are required.');if(c.users.some((x:any)=>lower(x.id)===lower(id)||lower(x.username)===lower(id)))fail('That username already exists.');if(c.users.some((x:any)=>lower(x.email)===email))fail('That email address already exists.');const user={id,username:id,name:clean(body.name)||id,email,position:clean(body.position),department:clean(body.department),companyId,role:[ROLE_ADMIN,ROLE_EMPLOYEE].includes(role)?role:ROLE_EMPLOYEE,profilePhoto:'',passwordHash:hashPassword(password),hiddenFromDirectory:false,active:true,createdAt:now(),createdBy:actor.id};c.users.push(user);await writeControl(c);return {status:201,body:publicUser(user)};
  }
  let m=path.match(/^\/api\/control-center\/users\/([^/]+)$/);
  if(m&&method==='DELETE'){
    requireRole(actor,[ROLE_DEVELOPER]);const id=decodeURIComponent(m[1]);if(lower(id)==='dvp')fail('Developer account cannot be deleted.');const c=await readControl();c.users=c.users.filter((x:any)=>x.id!==id);await writeControl(c);return {status:200,body:{deleted:true,id}};
  }
  m=path.match(/^\/api\/control-center\/users\/([^/]+)\/role$/);
  if(m&&method==='PATCH'){
    requireRole(actor,[ROLE_DEVELOPER,ROLE_ADMIN]);const id=decodeURIComponent(m[1]),c=await readControl(),u=c.users.find((x:any)=>x.id===id);if(!u)fail('User not found.',404);if(actor.role===ROLE_ADMIN&&u.companyId!==actor.companyId)fail('Administrators may only manage users in their company.',403);for(const k of ['role','position','department','supervisor','supervisoryRole'])if(body[k]!==undefined)u[k]=body[k];await writeControl(c);return {status:200,body:publicUser(u)};
  }
  if(path==='/api/control-center/documents'&&method==='POST'){
    const c=await readControl();const companyId=actor.role===ROLE_DEVELOPER?clean(body.companyId)||'COMPANY-DEFAULT':actor.companyId;const employeeId=actor.role===ROLE_EMPLOYEE?actor.id:clean(body.employeeId)||actor.id;const row={id:uid('DOC'),name:clean(body.name)||'Document',type:clean(body.type)||'application/octet-stream',size:Number(body.size||0),data:body.data||'',employeeId,employeeName:clean(body.employeeName)||actor.name,department:clean(body.department),projectCode:clean(body.projectCode),companyId,status:'PENDING_REVIEW',source:clean(body.source)||'Upload',sourceRef:clean(body.sourceRef),revision:Number(body.revision||1),createdAt:now(),uploadedBy:actor.id,assignedTo:''};c.documents.unshift(row);await writeControl(c);return {status:201,body:{...row,data:undefined}};
  }
...
Deno.serve(async (req:Request)=>{
  ...
  try{
    // The project uses Supabase's new sb_publishable_* browser key. Built-in
    // legacy JWT verification must therefore be OFF for this function. We
    // validate the apikey ourselves before any application route runs.
    if(!validClientApiKey(req)) return reply(req,{error:'Invalid or missing Supabase publishable key.'},401);
