-- ASSURANCE REGENT — SUPABASE-ONLY SETUP
-- Idempotent schema for Supabase Edge Function persistence.
...
-- These tables are private application data. They are accessed by the hosted
-- assurance-regent-api Edge Function with a privileged Supabase credential.
-- RLS is enabled with no anonymous browser policies.
...
create table if not exists public.agent_learning_mappings (
  ...
  unique(company_id, activity_key, project_code)
);

alter table public.agent_learning_mappings add column if not exists company_id text not null default 'COMPANY-DEFAULT';
alter table public.agent_learning_mappings drop constraint if exists agent_learning_mappings_activity_key_project_code_key;
create unique index if not exists agent_learning_company_activity_project_uidx on public.agent_learning_mappings(company_id, activity_key, project_code);
create index if not exists agent_learning_company_idx on public.agent_learning_mappings(company_id, last_confirmed_at desc);
...
-- Browser users do not access these tables directly. The Edge Function uses
-- Supabase's hosted privileged credential for controlled reads and writes.
...
-- The hosted Edge Function owns access to these tables. Application sign-in/session
-- authorization is enforced inside that function; no anonymous table policies are created.
...
alter table public.mts_work_sessions add column if not exists company_id text not null default 'COMPANY-DEFAULT';
create index if not exists mts_work_sessions_company_idx on public.mts_work_sessions(company_id, work_date);
...
alter table public.mts_messages add column if not exists company_id text not null default 'COMPANY-DEFAULT';
...
-- These tables remain private. All application access is brokered by the hosted
-- Edge Function, which enforces application role and company boundaries.
...
-- Assurance Regent — full mutable-state persistence and durable application sessions.
...
-- The Edge Function uses privileged Supabase credentials. Browser users receive no direct table policies.
...
-- OpenAI is intentionally NOT stored in Postgres/Vault by this build.
-- Configure OPENAI_API_KEY in Supabase Dashboard -> Edge Function Secrets.
-- The assurance-regent-api Edge Function reads it with Deno.env.get('OPENAI_API_KEY').

comment on table public.app_state_documents is 'Supabase Edge Function JSONB persistence for Assurance Regent mutable application state.';
comment on table public.app_auth_sessions is 'Supabase Edge Function-only hashed bearer-session records. Raw session tokens are never stored.';
