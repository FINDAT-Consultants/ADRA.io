...
const htmlTargets=[resolve(root,'index.html')],appTargets=[resolve(root,'app.js')],styleTargets=[resolve(root,'styles.css')];
...
let htmlSeen=0,appSeen=0,styleSeen=0;
...
for(const file of appTargets.filter(existsSync)){
  const source=readFileSync(file,'utf8');appSeen++;
  for(const token of ['RECRUITMENT_SELECTIONS_VISIBLE_SCHEMA107','ensureRecruitSelectionsVisible107','forceVisible:true','runtimeRecovery:true','RECRUITMENT_SELECTIONS_SCHEMA105','renderRecruitSelections105'])if(!source.includes(token))throw new Error(`Final Selections runtime missing ${token} in ${basename(file)}.`);
}
...
if(!htmlSeen||!appSeen||!styleSeen)throw new Error('Final Selections visibility verifier could not find all client target classes.');
console.log(`[verify-recruitment-selections-visible-v6-3-107] html=${htmlSeen} app=${appSeen} styles=${styleSeen} tab=visible order=funnel>selections>analytics runtime-recovery=enabled`);
