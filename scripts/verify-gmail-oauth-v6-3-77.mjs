...
for(const token of ['https://accounts.google.com/o/oauth2/v2/auth','https://oauth2.googleapis.com/token','https://gmail.googleapis.com/gmail/v1/users/me/messages/send','https://www.googleapis.com/auth/gmail.send',"action==='authorize_url'", "action==='status'", "action==='disconnect'", "action==='send'", "['recruitment','jivan'].includes(source)"])if(!edge.includes(token))throw new Error(`Gmail Edge Function behavior missing: ${token}`);
if(platformMode){
  for(const token of ["const PLATFORM_ACTOR_ID='Dvp'",'async function platformConnection()',"Email service is currently unavailable. Contact the Developer.","Developer authority is required to manage API connections.","connection_scope:'PLATFORM'"])if(!edge.includes(token))throw new Error(`Platform Gmail behavior missing: ${token}`);
  if(edge.includes('GMAIL_NOT_CONNECTED')||edge.includes('GMAIL_RECONNECT_REQUIRED'))throw new Error('Platform Gmail connector must not return per-user OAuth prompts.');
}else{
  for(const token of ['GMAIL_NOT_CONNECTED','GMAIL_RECONNECT_REQUIRED'])if(!edge.includes(token))throw new Error(`Legacy Gmail Edge Function behavior missing: ${token}`);
}
...
console.log(platformMode?'[gmail-oauth-verify] OK: Google Workspace OAuth is Developer-managed and shared platform-wide.':'[gmail-oauth-verify] OK: Gmail OAuth connect/disconnect/status UI and popup callback flow are present.');
