...
for(const token of ['https://accounts.google.com/o/oauth2/v2/auth','https://oauth2.googleapis.com/token','https://gmail.googleapis.com/gmail/v1/users/me/messages/send','https://www.googleapis.com/auth/gmail.send',"action==='authorize_url'", "action==='status'", "action==='disconnect'", "action==='send'", "['recruitment','jivan'].includes(source)",'GMAIL_NOT_CONNECTED','GMAIL_RECONNECT_REQUIRED'])if(!edge.includes(token))throw new Error(`Gmail Edge Function behavior missing: ${token}`);
