...
const required = [
  ["if(role==='Developer')return 'DEVELOPER'", 'Developer authority detector'],
  ["return 'HR_MANAGER'", 'HR Manager authority detector'],
  ["return 'FINANCE_MANAGER'", 'Finance Manager authority detector'],
  ["return 'PROGRAMS_MANAGER'", 'Programs Manager/Director authority detector'],
  ["return 'PROJECT_MANAGER'", 'Project Manager authority detector'],
  ["return 'HEAD_OF_DEPARTMENT'", 'Head of Department authority detector'],
  ["return 'SUPERVISOR'", 'Supervisor authority detector'],
  ["if(role==='Administrator')return 'ADMINISTRATOR'", 'Administrator fallback authority'],
  ["allowedViews:['dashboard','work','time','leave','calendar']", 'ordinary employee daily-work policy'],
  ["authority==='HR_MANAGER'", 'HR permission profile'],
  ["'recruiting','onboarding'", 'HR recruitment/onboarding access'],
  ["authority==='FINANCE_MANAGER'", 'Finance permission profile'],
  ["'payroll','calendar','monthly','checks','exceptions','assurance','voucher','audit'", 'Finance controls'],
...
for (const [needle, label] of required) {
  if (!source.includes(needle)) throw new Error(`RBAC verification failed: ${label} is missing.`);
}

if (source.includes("role:'Employee',allowedViews:['dashboard','company','work','time','leave','employees','calendar']")) {
  throw new Error('RBAC verification failed: ordinary employees still expose Company/Employees management views.');
}

console.log('[rbac-verify] OK: functional authority detection is present.');
