...
for(const file of files.filter(existsSync)){
  ...
  for(const token of ['LEAVE_WORK_STATUS_SCHEMA95','WORK_STATUS_VIEW95','resetLeaveRequestForm95','form.reset()','Submitting…','Select leave type','ensureWorkStatusView95','nav.dataset.view=WORK_STATUS_VIEW95','renderWorkStatusView95','Select work status','assurance_regent_browser_work_status_set','renderDashboardLeave();','The dashboard now reflects the latest location/status.'])if(!source.includes(token))throw new Error(`v6.3.95 runtime verification failed: ${token} missing from ${file}`);
