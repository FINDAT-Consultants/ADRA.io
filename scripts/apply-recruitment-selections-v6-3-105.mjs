...
for(const file of appTargets.filter(existsSync)){
  ...
  if(runtimeBlock.test(source))source=source.replace(runtimeBlock,runtime);else{
    const preferred='  /* Assurance Regent v6.3.100 — uninterrupted Zari primary voice lifecycle END */';
    const fallback='  /* Assurance Regent v6.3.96 — guaranteed Work Status sidebar END */';
    const anchor=source.includes(preferred)?preferred:fallback;
    if(!source.includes(anchor))throw new Error(`Recruitment selections v6.3.105 requires the post-work-status extension anchor in ${basename(file)}.`);
