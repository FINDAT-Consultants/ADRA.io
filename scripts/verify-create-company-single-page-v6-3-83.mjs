...
for(const forbidden of ['Next: Currency & country','data-developer-company-create-next82','data-developer-company-create-back82','developer-company-create-progress82','<b>Currency</b>','Select the currency this company will use.'])if(runtime.includes(forbidden))throw new Error(`Create Company v6.3.83 must not contain removed/wizard behavior: ${forbidden}`);
...
console.log('[create-company-v83-verify] OK: visible currency label/helper copy is removed while the dropdown remains accessible and functional.');
