...
const appPath=join(publicDir,appName),app=readFileSync(appPath,'utf8'),runtimePath=resolve(root,'scripts/meet-interview-assistant-v6-3-78-runtime.inc.js'),runtime=readFileSync(runtimePath,'utf8'),edge=readFileSync(resolve(root,'supabase/functions/meet-interview-assistant/index.ts'),'utf8'),gmail=readFileSync(resolve(root,'supabase/functions/gmail-connector/index.ts'),'utf8'),sql=readFileSync(resolve(root,'supabase/MEET_INTERVIEW_ASSISTANT_V6_3_78.sql'),'utf8'),platformMode=edge.includes("const PLATFORM_ACTOR_ID='Dvp'");
for(const token of ['MEET_READ_SCOPE78','data-jivan-interview-notes78','data-meet-compare78','meet-interview-assistant','authorizeMeetScope78','meetWaitForScope78','Advisory evidence only'])if(!app.includes(token))throw new Error(`Published Meet interview foundation missing: ${token}`);
...
for(const token of ['https://www.googleapis.com/auth/meetings.space.readonly','conferenceRecords?filter=','/participants?pageSize=250','/transcripts?pageSize=100','/entries?pageSize=100','TRANSCRIPT_NOT_READY','ranking_provided:false','human_decision_required'])if(!edge.includes(token))throw new Error(`Meet Edge Function behavior missing: ${token}`);
if(platformMode){
  for(const token of ["const PLATFORM_ACTOR_ID='Dvp'",'async function platformConnection()',"Jivan interview notes are currently unavailable. Contact the Developer.",'shared:true','api_key_configured','jivan_configured'])if(!edge.includes(token))throw new Error(`Platform Meet behavior missing: ${token}`);
  if(edge.includes('MEET_SCOPE_REQUIRED')||edge.includes('needs_auth:true'))throw new Error('Platform Meet assistant must not expose a per-user OAuth authorization requirement.');
}else if(!edge.includes('MEET_SCOPE_REQUIRED'))throw new Error('Legacy Meet scope requirement is missing.');
...
const appCheck=spawnSync(process.execPath,['--check',appPath],{encoding:'utf8'});if(appCheck.status!==0)throw new Error(`Published app syntax failure:\n${appCheck.stderr||appCheck.stdout}`);
const runtimeCheck=spawnSync(process.execPath,['--check',runtimePath],{encoding:'utf8'});if(runtimeCheck.status!==0)throw new Error(`Meet client runtime syntax failure:\n${runtimeCheck.stderr||runtimeCheck.stdout}`);
console.log('[meet-interview-assistant-verify] OK: Google Meet transcripts feed structured Jivan interview notes.');
console.log(platformMode?'[meet-interview-assistant-verify] OK: Meet access uses the Developer-managed shared Google Workspace connection.':'[meet-interview-assistant-verify] OK: Meet authorization waits specifically for the Meet transcript scope.');
