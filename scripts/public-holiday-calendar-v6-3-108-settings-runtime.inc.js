  ...
  function publicHolidayInstallSettingsControl108(){
    const currencySelect=$('settingsCurrencyCountry');if(!currencySelect)return;const generalSection=$('settingsGeneralSection')||currencySelect.closest('[data-settings-page="general"]')||currencySelect.closest('.control-settings-section');if(!generalSection)return;
    let section=$('settingsHolidayCalendarSection108');
    if(!section){section=document.createElement('div');section.id='settingsHolidayCalendarSection108';section.dataset.holidaySettingsGeneral='true';section.innerHTML='<div class="settings-section-head settings-holiday-subhead108"><div><b>Public holiday calendar</b><small>Select the country whose official public holidays appear on the operational calendar. By default it follows the organization country above.</small></div></div><label class="settings-holiday-follow108"><input id="settingsHolidayFollowCountry108" type="checkbox" /> Follow organization country automatically</label><label>Holiday calendar country<select id="settingsHolidayCountry108"><option value="">Choose holiday country</option></select></label><div class="currency-current" id="settingsHolidayCountryStatus108">Choose the organization country and currency above first.</div><button type="button" class="btn small ghost" id="settingsRefreshHolidayCalendar">Refresh public holidays now</button>';generalSection.appendChild(section);}
    if(section.parentElement!==generalSection)generalSection.appendChild(section);section.classList.add('settings-holiday-general108');section.style.marginTop='18px';section.style.paddingTop='16px';section.style.borderTop='1px solid var(--line,#dce6eb)';
    const s=state.control?.settings||{},follow=$('settingsHolidayFollowCountry108'),select=$('settingsHolidayCountry108'),directory=publicHolidayCountryDirectory108(),mode=String(s.holidayCountryMode||'follow').toLowerCase()==='override'?'override':'follow',selected=mode==='override'?(String(s.holidayCountryCode||s.countryCode||'').toUpperCase()):String(s.countryCode||s.holidayCountryCode||'').toUpperCase();if(!follow||!select)return;
  ...
  window.ADRAHolidayCalendarSettings={schema:PUBLIC_HOLIDAY_SETTINGS_SCHEMA108,install:publicHolidayInstallSettingsControl108,capture:publicHolidayCaptureSettings108,followOrganizationCountry:true,overrideSupported:true,generalSection:true,modalGeneralTarget:true};
