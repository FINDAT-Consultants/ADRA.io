  ...
  function publicHolidayInstallSettingsControl108(){
    const currencySelect=$('settingsCurrencyCountry');if(!currencySelect)return;let section=$('settingsHolidayCalendarSection108');
    if(!section){section=document.createElement('section');section.id='settingsHolidayCalendarSection108';section.className='control-settings-section';section.innerHTML='<div class="settings-section-head"><div><b>Public holiday calendar</b><small>Select which country supplies the official public holidays shown on the operational calendar. By default this follows the organization country selected above.</small></div></div><label class="settings-holiday-follow108"><input id="settingsHolidayFollowCountry108" type="checkbox" /> Follow organization country automatically</label><label>Holiday calendar country<select id="settingsHolidayCountry108"></select></label><div class="currency-current" id="settingsHolidayCountryStatus108"></div><button type="button" class="btn small ghost" id="settingsRefreshHolidayCalendar">Refresh public holidays now</button>';const currencySection=currencySelect.closest('.control-settings-section');currencySection?.insertAdjacentElement('afterend',section);}
    ...
    const canEdit=!Boolean(currencySelect.disabled);follow.disabled=!canEdit;select.disabled=!canEdit||follow.checked;
  ...
  window.ADRAHolidayCalendarSettings={schema:PUBLIC_HOLIDAY_SETTINGS_SCHEMA108,install:publicHolidayInstallSettingsControl108,capture:publicHolidayCaptureSettings108,followOrganizationCountry:true,overrideSupported:true};
