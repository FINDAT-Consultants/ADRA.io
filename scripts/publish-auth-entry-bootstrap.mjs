...
const bootstrap = `(() => {
  ...
  const installDialogAdapter = (dialog) => {
    ...
    // Authentication must not participate in the native <dialog> top layer.
    // Native modal stacking was the source of the visible-but-unclickable form.
    try {
  ...
  const normalizeControls = (dialog) => {
    ...
    dialog.querySelectorAll('input,select,button,textarea').forEach((control) => {
      if (control.dataset.authRuntimeDisabled === 'true') {
        control.disabled = false;
        delete control.dataset.authRuntimeDisabled;
      }
    });
  };
  ...
  const bindEntryControls = () => {
    ...
    // Prevent method=dialog from closing authentication before the application
    // submit handler has completed its Supabase request. Propagation is kept so
    // the canonical application authentication handlers still execute.
  ...
  const start = () => {
    ...
    observer.observe(document.body || document.documentElement, {
      subtree: true,
      attributes: true,
      attributeFilter: ['open', 'inert', 'class', 'disabled'],
      childList: true,
    });
