...
const staleWrites=[
  'if(cells[6])cells[6].innerHTML=workActivityProgressMarkup73(row);',
  'if(cells[10])cells[10].innerHTML=workActivityRateMarkup73(row);',
  "if(cells[11])cells[11].innerHTML=['completed','rework_required'].includes(row.status)?workActivityCostMarkup73(row):'—';"
];
for(const file of targets.filter(existsSync)){
  ...
  for(const stale of staleWrites)s=s.split(stale).join('');
  ...
  if(s.includes('cells[6].innerHTML=workActivityProgressMarkup73(row)'))throw new Error(`Clock-Out progress regression still exists in ${basename(file)}.`);
  ...
  if(s!==before)writeFileSync(file,s,'utf8');console.log(`[work-activity-table-columns] ${basename(file)} clock-out=time-only progress=progress-column stale-index-writes=removed`);
}
