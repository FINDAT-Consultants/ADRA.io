...
await import('./apply-developer-company-country-flags-v6-3-114.mjs');

const root=process.cwd(),publicDir=resolve(root,'public'),runtimePath=resolve(root,'scripts/calendar-holiday-labels-v6-3-115-app-runtime.inc.js');
if(!existsSync(runtimePath))throw new Error('Calendar holiday label runtime v6.3.115 is missing.');
const runtime=readFileSync(runtimePath,'utf8').trimEnd(),block=/  \/\* Assurance Regent v6\.3\.115 — human-readable calendar holiday labels START \*\/[\s\S]*?  \/\* Assurance Regent v6\.3\.115 — human-readable calendar holiday labels END \*\//u;
...
for(const file of targets.filter(existsSync)){
  let source=readFileSync(file,'utf8'),before=source;if(block.test(source))source=source.replace(block,runtime);else{const anchor='  /* Assurance Regent v6.3.114 — reliable visible country flags END */';if(!source.includes(anchor))throw new Error(`Calendar holiday labels require v6.3.114 in ${basename(file)}.`);source=source.replace(anchor,`${anchor}\n\n${runtime}`);}
  for(const token of ['CALENDAR_HOLIDAY_LABEL_SCHEMA115','1 January - Holiday: New Year','calendarHolidayDescription115','technicalMetadataPreserved:true','format:\'D Month - Holiday: Name\''])if(!source.includes(token))throw new Error(`Calendar holiday label runtime missing ${token} in ${basename(file)}.`);
  const check=spawnSync(process.execPath,['--check',file],{encoding:'utf8'});if(check.status!==0)throw new Error(`Calendar holiday label syntax failure in ${basename(file)}:\n${check.stderr||check.stdout}`);if(source!==before)writeFileSync(file,source,'utf8');
}
...
if(publicApps.length&&sriUpdated<1)throw new Error(`Calendar holiday label patch updated ${publicApps.length} public app runtime(s) but found no HTML app-script SRI binding to refresh.`);
console.log(`[calendar-holiday-labels-v6-3-115] apps=${targets.filter(existsSync).length} format="D Month - Holiday: Name" sri-bindings=${sriUpdated}`);
await import('./verify-calendar-holiday-labels-v6-3-115.mjs');
