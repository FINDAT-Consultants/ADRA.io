...
for(const file of targets.filter(existsSync)){
  ...
  for(const token of ['ONBOARDING_COMPLETION_TRANSFER_SCHEMA118','onboardingActiveRows118','capturePhaseOverride:true','installOnboardingCompletionCapture118','flushStandaloneSaveFully118','standaloneSavePending','authoritativeOnboardingReadback118','persistAndVerifyOnboardingCompletion118','retryUntilConfirmed:true','completedRecordsRetainedForAudit:true','persistedBeforeReload:true','removedFromActiveQueue:true','has moved to Employees','is now an active employee'])if(!source.includes(token))throw new Error(`Onboarding completion transfer runtime missing ${token} in ${basename(file)}.`);
...
console.log(`[onboarding-completion-transfer-v6-3-118] apps=${targets.filter(existsSync).length} active-queue-only=1 capture-phase=1 drain-save-queue=1 authoritative-readback=1 retry-until-confirmed=1 employee-transfer=1 sri-bindings=${sriUpdated}`);
