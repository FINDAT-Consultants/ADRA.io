...
const jsName='work-analytics-layout-v6-3-126.js';
const cssName='work-analytics-layout-v6-3-126.css';
...
for(const file of [sourceJs,sourceCss,rootIndex,publicIndex])if(!existsSync(file))throw new Error(`Required v6.3.126 input is missing: ${file}`);
const syntax=spawnSync(process.execPath,['--check',sourceJs],{encoding:'utf8'});
if(syntax.status!==0)throw new Error(syntax.stderr||syntax.stdout||'v6.3.126 runtime syntax check failed.');
...
function patch(file,{publicArtifact=false}={}){
  ...
  const stability=/<script\b[^>]*\bsrc=["']\.\/interface-stability-v6-3-125\.js(?:\?[^"']*)?["'][^>]*><\/script>/iu;
  const country=/<script\b[^>]*\bsrc=["']\.\/country-select-v6-3-124\.js(?:\?[^"']*)?["'][^>]*><\/script>/iu;
  const app=/<script\b[^>]*\bsrc=["']\.\/app(?:\.[^"'?]+)?\.js(?:\?[^"']*)?["'][^>]*><\/script>/iu;
  if(stability.test(html))html=html.replace(stability,m=>`${m}\n${scriptTag}`);
  else if(country.test(html))html=html.replace(country,m=>`${m}\n${scriptTag}`);
...
if(!published.includes(`src="./${jsName}?v=${jsV}"`)||!published.includes(`integrity="${sha384(publicJs)}"`))throw new Error('Published v6.3.126 JS binding failed.');
