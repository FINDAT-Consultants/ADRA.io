...
for(const token of ['BUDGET_PERSONNEL_SCHEMA87','Personnel Directory','Developer account is excluded','budgetCategory','EMPLOYEE','Template downloaded ·'])if(!app.includes(token))throw new Error(`Built app is missing budget personnel v6.3.87 token: ${token}`);
...
for(const token of ['assurance_regent_browser_budget_personnel_exists','assurance_regent_browser_budget_personnel_directory','auth\'->\'accounts','budgetCategory','EMPLOYEE','UNKNOWN_PERSONNEL','ALL_APPROVED_COMPANY_ACCOUNTS_EXCEPT_DEVELOPER'])if(!sql.includes(token.replaceAll('\\','')))throw new Error(`Budget personnel migration missing ${token}`);
