...
const css=resolve(root,'company-social-experience-v6-3-63.css'),compactCss=resolve(root,'company-social-analytics-compact-v6-3-64.css'),runtime=resolve(root,'scripts/company-social-experience-v6-3-63-runtime.inc.js');
if(!existsSync(css)||!existsSync(compactCss)||!existsSync(runtime))throw new Error('Company social experience assets are missing.');
if(existsSync(publicDir)){writeFileSync(join(publicDir,'company-social-experience-v6-3-63.css'),readFileSync(css,'utf8'),'utf8');writeFileSync(join(publicDir,'company-social-analytics-compact-v6-3-64.css'),readFileSync(compactCss,'utf8'),'utf8');}

function patchHtml(file){let s=readFileSync(file,'utf8'),before=s;s=s.replace(/\s*<link rel="stylesheet" href="\.\/company-social-experience-v6-3-63\.css\?v=[^"]+" \/>/gu,'').replace(/\s*<link rel="stylesheet" href="\.\/company-social-analytics-compact-v6-3-64\.css\?v=[^"]+" \/>/gu,'');const link='  <link rel="stylesheet" href="./company-social-experience-v6-3-63.css?v=6.3.63" />\n  <link rel="stylesheet" href="./company-social-analytics-compact-v6-3-64.css?v=6.3.64" />',anchor='<link rel="stylesheet" href="./department-hub-stories-trending.css?v=6.3.62" />';if(s.includes(anchor))s=s.replace(anchor,anchor+'\n'+link);else s=s.replace('</head>',link+'\n</head>');if(s!==before)writeFileSync(file,s,'utf8');console.log(`[company-social-experience] ${basename(file)} trend-chart=tight-compact status-carousel=horizontal developer-company-selector=enabled`);}
