  ...
  function publicHolidayEnhanceCalendarDots108(){
    ...
    rows.forEach((row,i)=>{const holiday=String(row.dayType||'').toLowerCase()==='holiday',tr=trs[i],card=cards[i];if(!holiday){tr?.classList.remove('public-holiday-row108');return;}const name=publicHolidayName108(row)||'Public holiday';tr?.classList.add('public-holiday-row108');
      if(tr?.cells?.[2]){let label=tr.cells[2].querySelector('.public-holiday-label108');if(!label){label=document.createElement('span');label.className='public-holiday-label108';label.append(document.createElement('i'),document.createTextNode('Holiday'));tr.cells[2].prepend(label,document.createElement('br'));}publicHolidayEnsureDot108(label);tr.title=`${row.date} — ${name}`;}
      if(card){let marker=card.querySelector('.public-holiday-marker108');if(!marker){marker=document.createElement('small');marker.className='public-holiday-marker108';marker.append(document.createElement('i'),document.createTextNode(name));card.insertBefore(marker,card.children[2]||null);}publicHolidayEnsureDot108(marker);marker.lastChild&&marker.lastChild.nodeType===3&&(marker.lastChild.textContent=name);card.title=`${row.date} — ${name}`;}
    });
  }
