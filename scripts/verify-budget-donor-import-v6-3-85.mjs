...
const root=process.cwd(),app=resolve(root,'app.js'),runtime=resolve(root,'scripts/budget-donor-import-v6-3-85-runtime.inc.js'),sql=resolve(root,'supabase/BUDGET_DONOR_IMPORT_V6_3_85.sql'),windowSql=resolve(root,'supabase/BUDGET_DONOR_IMPORT_V6_3_85_RECOVERY_WINDOW.sql'),pkg=resolve(root,'package.json');
for(const file of [app,runtime,sql,windowSql,pkg])if(!existsSync(file))throw new Error(`Missing required v6.3.85 file: ${file}`);
const appText=readFileSync(app,'utf8'),runtimeText=readFileSync(runtime,'utf8'),sqlText=readFileSync(sql,'utf8'),windowSqlText=readFileSync(windowSql,'utf8'),pkgText=readFileSync(pkg,'utf8');
...
for(const token of ['xlsx-0.20.3','Project Budget','Personnel Rates','Donor Rules','The uploader cannot approve their own batch.','COUNTRY_APPROVE'])if(!runtimeText.includes(token))throw new Error(`Runtime is missing v6.3.85 token: ${token}`);
for(const token of ['assurance_regent_budget_import_batches','assurance_regent_budget_import_projects','assurance_regent_budget_import_rates','assurance_regent_budget_import_rules','PENDING_FINANCE_REVIEW','PENDING_COUNTRY_DIRECTOR','security definer','enable row level security','activeBudgets','activeRates','Maker-checker control'])if(!sqlText.includes(token))throw new Error(`Migration is missing v6.3.85 token: ${token}`);
for(const token of ['ar_budget_import_superseded_by_idx','v_month_end','v_month_start','activeBudgets','activeRates','date_trunc'])if(!windowSqlText.includes(token))throw new Error(`Recovery-window migration is missing v6.3.85 token: ${token}`);
