...
for(const file of targets.filter(existsSync)){
  ...
  for(const token of ['ONBOARDING_COMPLETION_TRANSFER_SCHEMA118','onboardingActiveRows118','String(o?.status||\'\').trim().toLowerCase()!==\'complete\'','No active onboarding records.','Completed hires are available in Employees.','await flushStandaloneSave()','state.onboardingSelected=\'\'','renderEmployees();renderCompany();','switchView(\'employees\')','employeeCreated:Boolean(employee)','candidateHired','removedFromActiveQueue:true','persistedBeforeReload:true','navigatedToEmployees:Boolean(employeesOpened)','completedRecordsRetainedForAudit:true','autoOpenEmployees:true'])if(!source.includes(token))throw new Error(`Onboarding completion verifier missing ${token} in ${basename(file)}.`);
...
console.log(`[verify-onboarding-completion-transfer-v6-3-118] OK apps=${targets.filter(existsSync).length} active-queue-only=1 employee-transfer=1 candidate-hired=1 auto-open-employees=1 persisted-before-reload=1 sri-bindings=${sriBindings}`);
