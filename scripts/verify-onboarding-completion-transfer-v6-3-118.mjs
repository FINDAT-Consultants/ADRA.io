...
for(const file of targets.filter(existsSync)){
  ...
  for(const token of ['ONBOARDING_COMPLETION_TRANSFER_SCHEMA118','6.3.118-hard','onboardingActiveRows118','String(o?.status||\'\').trim().toLowerCase()!==\'complete\'','No active onboarding records.','Completed hires are available in Employees.','installOnboardingCompletionCapture118','stopImmediatePropagation','capturePhaseOverride:true','flushStandaloneSaveFully118','standaloneSavePending','standaloneSaveInFlight','authoritativeOnboardingReadback118','loadStandaloneState','persistAndVerifyOnboardingCompletion118','onboardingPersisted118','removedFromActiveQueue=!active','legacyComplete=Boolean(active)','retryUntilConfirmed:true','successRequiresEmployeeAndHiredCandidate:true','successAcceptsArchivedRemoval:true','state.onboardingSelected=\'\'','state.employeeDirectoryFilter=\'\'','switchView(\'employees\')','employeeCreated:Boolean(result.employee)','candidateHired:result.candidateHired','removedFromActiveQueue:result.removedFromActiveQueue||result.legacyComplete','persistedBeforeReload:true','authoritativeReadback:true','completedRecordsRetainedForAudit:true','autoOpenEmployees:true'])if(!source.includes(token))throw new Error(`Onboarding completion verifier missing ${token} in ${basename(file)}.`);
  for(const token of ['engine.upsertOnboarding','engine.upsertCandidate','engine.upsertEmployee','status:\'Hired\'','status:\'Complete\''])if(!source.includes(token))throw new Error(`Onboarding completion reconciliation contract missing ${token} in ${basename(file)}.`);
}
...
console.log(`[verify-onboarding-completion-transfer-v6-3-118] OK apps=${targets.filter(existsSync).length} active-queue-only=1 archived-removal-success=1 capture-phase=1 drain-save-queue=1 authoritative-readback=1 retry-until-confirmed=1 employee-transfer=1 candidate-hired=1 auto-open-employees=1 sri-bindings=${sriBindings}`);
