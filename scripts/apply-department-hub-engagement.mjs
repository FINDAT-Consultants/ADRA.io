...
function patchApp(file){let s=readFileSync(file,'utf8'),before=s,addon=readFileSync(runtime,'utf8');
  ...
  if(s.includes("[['notificationBadge',n],['documentBadge',d]"))s=s.replace("[['notificationBadge',n],['documentBadge',d]","[['notificationBadge',n+(state.companyHubNotifications||[]).length],['documentBadge',d]");
  ...
  if(s!==before)writeFileSync(file,s,'utf8');console.log(`[department-hub-engagement] ${basename(file)} comments=like-love-reply notifications=deep-link`);
}
