...
if(!existsSync(appRuntimeFile)||!existsSync(agentRuntimeFile))throw new Error('Public holiday calendar v6.3.108 runtime assets are missing.');
...
for(const file of appTargets.filter(existsSync)){
  ...
  for(const token of ['PUBLIC_HOLIDAY_CALENDAR_SCHEMA108','/api/v4/Holidays','publicHolidayEnsureReminderWindow108','ADRAHolidayCalendar','assurance-regent-holiday-ai-reminder'])if(!source.includes(token))throw new Error(`Public holiday calendar runtime missing ${token} in ${basename(file)}.`);
...
console.log('[public-holiday-calendar-v6-3-108] country-aware=enabled names=visible expected-hours=adjusted ai-reminders=visual-deduplicated provider=nager-v4');
