...
const settingsGeneralFile=resolve(root,'scripts/public-holiday-calendar-v6-3-108-settings-general.inc.html');
const agentRuntimeFile=resolve(root,'scripts/public-holiday-calendar-v6-3-108-agent-runtime.inc.js');
if(!existsSync(appRuntimeFile)||!existsSync(dotRuntimeFile)||!existsSync(dateMarkerFixRuntimeFile)||!existsSync(settingsRuntimeFile)||!existsSync(settingsGeneralFile)||!existsSync(agentRuntimeFile))throw new Error('Public holiday calendar v6.3.108 runtime assets are missing.');
const appRuntime=readFileSync(appRuntimeFile,'utf8').trimEnd(),dotRuntime=readFileSync(dotRuntimeFile,'utf8').trimEnd(),dateMarkerFixRuntime=readFileSync(dateMarkerFixRuntimeFile,'utf8').trimEnd(),settingsRuntime=readFileSync(settingsRuntimeFile,'utf8').trimEnd(),settingsGeneral=readFileSync(settingsGeneralFile,'utf8').trimEnd(),agentRuntime=readFileSync(agentRuntimeFile,'utf8').trimEnd();
...
const settingsGeneralBlock=/        <!-- Assurance Regent v6\.3\.108 — holiday country General settings START -->[\s\S]*?        <!-- Assurance Regent v6\.3\.108 — holiday country General settings END -->/u;
...
const htmlTargets=[resolve(root,'index.html')],appTargets=[resolve(root,'app.js')],agentTargets=[resolve(root,'recovery-agent-v5.js')];
if(existsSync(publicDir)){htmlTargets.push(resolve(publicDir,'index.html'));for(const name of readdirSync(publicDir)){
...
for(const file of htmlTargets.filter(existsSync)){
  ...
  if(settingsGeneralBlock.test(source))source=source.replace(settingsGeneralBlock,settingsGeneral);else{
    if(!source.includes('id="settingsGeneralSection"'))throw new Error(`Public holiday settings require the real Settings > General panel in ${basename(file)}.`);
    const anchor='<div class="currency-current" id="currencyCurrent"></div></section>';
    if(!source.includes(anchor))throw new Error(`Public holiday settings require the General currency anchor in ${basename(file)}.`);
    source=source.replace(anchor,`<div class="currency-current" id="currencyCurrent"></div>\n${settingsGeneral}\n        </section>`);
  }
  for(const token of ['id="settingsGeneralSection"','data-settings-page="general"','data-holiday-settings-general="true"','id="settingsHolidayFollowCountry108"','id="settingsHolidayCountry108"','id="settingsRefreshHolidayCalendar"'])if(!source.includes(token))throw new Error(`Public holiday General settings markup missing ${token} in ${basename(file)}.`);
...
for(const file of appTargets.filter(existsSync)){
  ...
  if(dateMarkerFixBlock.test(source))source=source.replace(dateMarkerFixBlock,dateMarkerFixRuntime);else{
    const anchor='  /* Assurance Regent v6.3.108 — public holiday calendar dot markers END */';
...
console.log('[public-holiday-calendar-v6-3-108] settings-general=static-and-runtime-bound country-aware=enabled holiday-country-setting=enabled follow-organization-country=default override=enabled currency-context=enabled holiday-dots=yellow-date-bound names=tooltips expected-hours=adjusted ai-reminders=visual-deduplicated provider=nager-v4');
