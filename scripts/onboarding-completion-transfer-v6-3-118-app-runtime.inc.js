  ...
  function onboardingPersisted118(row={}){const saved=(engine.state.onboarding||[]).find(o=>String(o.id||'')===String(row.id||'')),employee=onboardingEmployeeFor118(row),candidate=onboardingCandidateFor118(row),candidateHired=!candidate||String(candidate.status||'').trim().toLowerCase()==='hired';return {saved,employee,candidate,candidateHired,ok:Boolean(saved)&&String(saved.status||'').trim().toLowerCase()==='complete'&&Boolean(employee)&&candidateHired};}
  ...
  moveOnboarding=async function(direction){
    ...
    try{
      ...
      window.dispatchEvent(new CustomEvent(ONBOARDING_COMPLETION_EVENT118,{detail:{schema:ONBOARDING_COMPLETION_TRANSFER_SCHEMA118,onboardingId:next.id||'',candidateId:next.candidateId||'',employeeId:next.employeeId||'',employeeCreated:Boolean(result.employee),candidateHired:result.candidateHired,removedFromActiveQueue:true,persistedBeforeReload:true,authoritativeReadback:true,retriedUntilConfirmed:true,navigatedToEmployees:Boolean(employeesOpened)}}));
  ...
  window.AssuranceRegentOnboardingCompletionTransfer={schema:ONBOARDING_COMPLETION_TRANSFER_SCHEMA118,activeQueueOnly:true,completedRecordsRetainedForAudit:true,employeeTransfer:true,candidateMarkedHired:true,persistedBeforeReload:true,autoOpenEmployees:true,capturePhaseOverride:true,drainSaveQueue:true,authoritativeReadback:true,retryUntilConfirmed:true,event:ONBOARDING_COMPLETION_EVENT118};
