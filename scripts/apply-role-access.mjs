...
const legacyEmployeePolicies=[
  "return {...base,role:'Employee',allowedViews:['dashboard','company','work','time','leave','employees','calendar']};}",
  "return {...base,role:'Employee',allowedViews:['dashboard','work','time','leave','calendar']};}"
];
const employeePolicy="return {...base,role:'Employee',allowedViews:['dashboard','company','work','time','leave','calendar']};}";
...
let changed=0;
for(const file of targets){let source=readFileSync(file,'utf8'),original=source;
  for(const old of legacyEmployeePolicies)if(source.includes(old)&&old!==employeePolicy)source=source.replace(old,employeePolicy);
  if(!source.includes(employeePolicy))throw new Error(`Employee permission policy was not found in ${basename(file)}.`);
  if(source.includes(oldSwitchStart))source=source.replace(oldSwitchStart,guardedSwitchStart);else if(!source.includes('v6.3.25 — enforce functional RBAC before rendering a protected view.'))throw new Error(`switchView RBAC insertion point was not found in ${basename(file)}.`);
  if(source.includes(oldNavVisibility))source=source.replace(oldNavVisibility,hardenedNavVisibility);else if(!source.includes('v6.3.26 — unauthorized sidebar features must not be rendered, focused, or exposed to assistive navigation.'))throw new Error(`Sidebar visibility RBAC insertion point was not found in ${basename(file)}.`);
  if(source!==original){writeFileSync(file,source,'utf8');changed++;}
  console.log(`[rbac] ${basename(file)} employee=personal-work-plus-company-directory direct-view-guard=enabled sidebar=fully-hidden-when-unauthorized`);
}
