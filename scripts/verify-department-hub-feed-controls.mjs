...
if(!html.includes('department-hub-composer-fit.css?v=6.3.58'))throw new Error('Department Hub single-card composer stylesheet is not linked.');
...
for(const token of ['#companyHubForm .social-composer-content','grid-template-columns:minmax(0,1fr) auto!important','#companyHubForm textarea','grid-column:1!important','#companyHubForm .company-social-composer-actions','display:contents!important','#companyHubForm .social-composer-submit','grid-column:2!important','position:static!important','inset:auto!important','.company-social-post .company-social-media-grid','background:transparent!important','.company-social-post .company-hub-inline-audio-post','width:100%!important','max-width:none!important','border:0!important','.company-social-post .company-social-document'])if(!fitCss.includes(token))throw new Error(`Department Hub single-card style missing: ${token}`);
if(fitCss.includes('position:absolute!important'))throw new Error('Department Hub Post button must never return to absolute/floating positioning.');
...
console.log('[department-hub-feed-controls-verify] OK: Post participates in the same composer grid row as the text field and is never absolutely positioned.');
console.log('[department-hub-feed-controls-verify] OK: attachment controls remain in a separate row beneath the composer input.');
console.log('[department-hub-feed-controls-verify] OK: root posts keep one visible card; media wrappers and inline audio are transparent layout surfaces rather than nested cards.');
console.log('[department-hub-feed-controls-verify] OK: inline audio retains play/pause, waveform, duration and draggable seek.');
