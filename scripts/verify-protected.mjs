...
if (careersFiles.length !== 1) {
  failures.push(`Expected exactly one public careers runtime, found ${careersFiles.length}.`);
}
...
for (const file of jsFiles) {
  ...
  if (careersFiles.includes(file)) {
    if (/_ar_[A-Za-z0-9_$]+/u.test(code)) {
      failures.push(`${relative(root, file)} was obfuscated even though the public vacancy/application runtime must remain stable.`);
    }
    if (!code.includes("call('list_vacancies'")) {
      failures.push(`${relative(root, file)} no longer contains the public vacancy loader.`);
    }
    if (!code.includes("/functions/v1/recruitment-public")) {
      failures.push(`${relative(root, file)} no longer targets the public recruitment service.`);
    }
    if (!code.includes('function render()')) {
      failures.push(`${relative(root, file)} no longer contains the vacancy renderer.`);
    }
  }
}
