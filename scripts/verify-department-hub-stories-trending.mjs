...
const appPath=join(p,appName),app=readFileSync(appPath,'utf8'),css=readFileSync(join(p,'department-hub-stories-trending.css'),'utf8'),viewRuntime=readFileSync(resolve(root,'scripts/social-analytics-view-tracking-v6-3-68-runtime.inc.js'),'utf8');
...
for(const token of ['companyHubReliableViewObserver68','companyHubReliableVisibleRatio68','intersectionRatio>=.42','companyHubReliableRecord68','qualified-feed','image-open','attachment-open','companyHubReliableMediaConsumed68','kind===\'video\'?1200:450','document.addEventListener(\'play\'','bindCompanyHubReliableViewTracking68'])if(!app.includes(token))throw new Error(`v6.3.68 view tracking runtime missing: ${token}`);
for(const token of ['reliable Department Hub social view tracking','companyHubReliableViewObserver68','qualified-feed','image-open','companyHubReliableMediaConsumed68'])if(!viewRuntime.includes(token))throw new Error(`v6.3.68 source runtime missing: ${token}`);
...
console.log('[department-hub-stories-trending-verify] OK: v6.3.68 records qualified feed visibility plus image, attachment, video and audio consumption into authenticated social analytics.');
