...
- Recovery Agent is restored through a Supabase Edge Function. Set OPENAI_API_KEY in Supabase Edge Function Secrets; the browser never receives the key. Chat is advisory/read-only in v4.7.0 and conversation history is stored per signed-in user in Supabase.
...
Run `RECOVERY_AGENT_SUPABASE_UPDATE.sql`, deploy `supabase/functions/recovery-agent/index.ts` as an Edge Function named `recovery-agent`, disable the legacy Verify JWT check for this function because Assurance Regent uses its own session token, and add `OPENAI_API_KEY` in Edge Function Secrets. Do not put the key in browser code or Git.
