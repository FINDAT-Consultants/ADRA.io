...
- **Developer** — hidden bootstrap identity with full application authority, company creation, account/data administration and role assignment. Fixed sign-in: `Dvp` with the fixed Developer password configured in this build. The Developer credential is not overridden by a deployment environment variable in v4.4.2.
...
## Supabase — required durable store

Assurance Regent now treats Supabase as the required source of truth for durable application data. Run migrations `001` through `005` (or paste `supabase/ASSURANCE_REGENT_COMPLETE_SETUP.sql` once in the Supabase SQL Editor). Migration `005_supabase_only_persistence.sql` adds the server-only JSONB application-state store and the private `assurance-regent-files` Storage bucket.

Durable browser `localStorage` persistence has been removed. User accounts, companies, settings, live operational state, AI advisor threads/activity, memory/records, learning mappings, MTS work sessions/messages, and controlled document metadata are persisted in Supabase. Uploaded controlled documents are stored in private Supabase Storage and opened through short-lived signed URLs. Live authentication uses a short-lived HttpOnly cookie; the server stores only its SHA-256 token hash and user binding in Supabase.

`SUPABASE_SECRET_KEY` is server-only and must be configured in the deployment environment, never committed to GitHub or embedded in browser JavaScript.
...
The root `index.html` can still be opened directly as a **non-persistent interface preview**. Live use requires the application server and sign-in. In live mode, Supabase is the required durable store for accounts, operational data, agent memory, work sessions, settings, reviews and controlled-document metadata; uploaded files use private Supabase Storage.
...
This means the live agent can continue a conversation across requests and server restarts through Supabase.
...
## Direct interface preview

1. Extract the ZIP to a normal folder.
2. Double-click the root `index.html` only when you want to inspect the interface.
3. Preview mode is intentionally non-persistent and does not save durable user/application data in the browser.

Full sign-in, Recovery Agent execution, persistent data and document storage require the deployed application server connected to Supabase.
...
- A host capable of running the Node.js application server
- A Supabase project for all durable backend storage
- An OpenAI API key only if Recovery Agent/OpenAI features are required

Configure the values shown in `.env.example` using your hosting provider's environment-variable/settings page. Do not put server secrets in GitHub or browser JavaScript.

```env
OPENAI_API_KEY=your_key
OPENAI_MODEL=gpt-5.6

# Required persistent backend
SUPABASE_URL=https://YOUR_PROJECT_REF.supabase.co
SUPABASE_SECRET_KEY=your_server_secret
```

Deploy the repository with your normal hosting provider workflow. The host must run the Node application entry point (`server.js`) and supply the environment values above. No browser-side secret configuration is used.

---

## Supabase setup

Use the Supabase Dashboard → SQL Editor. For an **existing live Assurance Regent project**, paste `supabase/ASSURANCE_REGENT_EXISTING_PROJECT_UPDATE.sql`. For a **brand-new/empty project**, paste `supabase/ASSURANCE_REGENT_COMPLETE_SETUP.sql`. The fresh combined file includes the supplied workbook seed and therefore is not the upgrade path for a live database.

The schema includes the workbook foundation tables, MTS work sessions/messages, persistent agent memories/conversation items/records/action logs/learning mappings, the server-only `assurance_regent_state` table, and the private `assurance-regent-files` Storage bucket.
...
The project retains its automated validation scripts in `package.json` for deployment/CI environments. The supplied ZIP was syntax-checked and secret-scanned before packaging. `verify-engine` validates key workbook results and `verify-memory` validates source integrity and seeded control knowledge.
...
The system automatically refreshes its advisory models when live operational state or human-confirmed learning changes.
...
Current trained-model metadata/state is stored in Supabase under the server-only `assurance_regent_state` key `intelligence-models`. The bundled `data/ml-models.json` is retained only as a historical/reference artifact and is not a live persistence target.
...
Create a fresh project key and configure it only in the server environment using your hosting provider's environment/settings page. The `.env.example` file shows the required variable names but contains no live secret values.
...
Automated verification scripts remain available to CI/deployment systems through the `package.json` scripts (`check`, `verify-engine`, `verify-memory`, `verify-unified`, `verify-intelligence`, and `verify-security`).
...
For live use, deploy the Node application through your hosting provider and configure `OPENAI_API_KEY` in that provider's environment/settings page when AI features are required.
