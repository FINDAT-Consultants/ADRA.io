/* Assurance Regent Recovery Agent v5.1 — synchronized floating operator + voice I/O */
(() => {
  'use strict';
  ...
  let recorder=null, stream=null, chunks=[], stopTimer=null, sending=false, voiceEnabled=true, currentAudio=null, pausedByMute=false;
  ...
  function buildUi(){
    ...
    panel.innerHTML=`
      ...
      <form class="raf-form" id="rafForm">
        ...
        <div class="raf-controls"><button type="button" class="raf-mic" id="rafMic" aria-label="Speak to Recovery Agent" title="Start microphone recording">🎙 Mic</button><button type="submit" class="raf-send">Send</button></div>
      </form>`;
  ...
  function refreshVisibility(){
    ...
    const hiddenForAgentPage=onAgentPage();
    launcher.hidden=!signed||hiddenForAgentPage;
    if(!signed||hiddenForAgentPage){panel.hidden=true;launcher.classList.remove('panel-open');}
  }

  function setOpen(open){
    if(!panel||!launcher)return;
    if(onAgentPage()){panel.hidden=true;launcher.hidden=true;launcher.classList.remove('panel-open');$('chatInput')?.focus();return;}
  ...
  async function speak(text){
    ...
    catch(err){console.warn('Recovery Agent voice unavailable:',err);setStatus('Spoken response could not play. Text response remains available.');}
  }

  function attachBridge(){bridge=window.AssuranceRegentAgentBridge||null;if(!bridge)return false;buildUi();threadSynced=false;refreshIdentity();return true;}
  ...
  window.addEventListener('assurance-regent-agent-response',e=>{const d=e.detail||{};refreshIdentity();const reply=d.response?.text;if(reply){setStatus(`Ready in ${viewName()}.`);speak(reply);}else if(d.error)setStatus(d.error);});
