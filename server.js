...
import { getControlCenter, getControlProfile, getControlUserScope, loginControlUser, registerControlUser, logoutControlUser, updateControlProfile, updateControlSettings, createCompany, createCompanyWithExecutive, assignControlRole, createControlUserByDeveloper, deleteControlUser, ingestControlDocument, reviewControlDocument, listControlReviews, actOnControlReview, saveAgentApprovalReview, resolveStoredControlReview, getControlDocumentContent } from './src/control-center-store.js';
...
import { assertSupabaseReady, bootstrapOpenAIFromSupabaseVault } from './src/supabase.js';
import { issueAuthSession, getAuthSession, revokeAuthSession, purgeExpiredAuthSessions } from './src/auth-session-store.js';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const app = express();
const port = Number(process.env.PORT || 3000);
const supabaseStatus=await assertSupabaseReady();
const openaiSecretStatus=await bootstrapOpenAIFromSupabaseVault();
...
app.get('/api/health', async (_req, res) => res.json({
  ok: true,
  storage: {supabase: Boolean(supabaseStatus?.configured), required: Boolean(supabaseStatus?.required), openai_secret_source: openaiSecretStatus?.source || 'none'},
...
app.use('/api',async(req,res,next)=>{
  try{
    ...
    const token=bearerToken(req),session=token?await getAuthSession(token,AUTH_TTL_MS):null;
...
app.post('/api/control-center/login', async (req,res,next)=>{try{const result=await loginControlUser(req.body||{}),token=await issueAuthSession(result.currentUser.id,AUTH_TTL_MS);res.json({...result,session_token:token});}catch(e){e.statusCode=401;next(e);}});
app.post('/api/control-center/register', async (req,res,next)=>{try{const result=await registerControlUser(req.body||{}),token=await issueAuthSession(result.currentUser.id,AUTH_TTL_MS);res.status(201).json({...result,session_token:token});}catch(e){e.statusCode=e.statusCode||400;next(e);}});
...
app.post('/api/control-center/logout', async (req,res,next)=>{try{if(req.authToken)await revokeAuthSession(req.authToken);res.json({signedIn:false});}catch(e){next(e);}});
...
app.get('/api/control-center/documents/:id/content', async (req,res,next)=>{try{res.json(await getControlDocumentContent(req.params.id,req.authUserId));}catch(e){next(e);}});
...
await purgeExpiredAuthSessions();
...
app.listen(port,()=>console.log(`ADRA Recovery Passport API server is running on port ${port}. Supabase persistence is active. OpenAI-powered agent tasks use OPENAI_API_KEY from Supabase Vault or the server environment and do not use a local-model fallback.`));
