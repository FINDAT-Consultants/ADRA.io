...
import { getControlCenter, getControlProfile, getControlUserScope, loginControlUser, registerControlUser, logoutControlUser, updateControlProfile, updateControlSettings, createCompany, createCompanyWithExecutive, assignControlRole, createControlUserByDeveloper, deleteControlUser, ingestControlDocument, reviewControlDocument, listControlReviews, actOnControlReview, saveAgentApprovalReview, resolveStoredControlReview } from './src/control-center-store.js';
...
app.get('/api/control-center/documents/:id/content', async (req,res,next)=>{try{const base=await getControlCenter(req.authUserId);const meta=base.documents.find(d=>d.id===req.params.id);if(!meta)return res.status(404).json({error:'Document not found.'});const fsmod=await import('node:fs/promises');const control=JSON.parse(await fsmod.readFile(path.join(__dirname,'data','control-center.json'),'utf8'));const doc=(control.documents||[]).find(d=>d.id===req.params.id);if(!doc)return res.status(404).json({error:'Document not found.'});res.json({id:doc.id,name:doc.name,type:doc.type,data:doc.data,status:doc.status,archivePath:doc.archivePath});}catch(e){next(e);}});
